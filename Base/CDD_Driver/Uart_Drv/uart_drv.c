...
#define UART_LIN_HEADER_DONE_FLG      ((uint32)1U << 14U)
#define UART_LIN_RSP_DONE_FLAG        ((uint32)1U << 15U)
#define UART_LIN_TO_ERR               ((uint32)1U << 13U)
#define UART_LIN_CHECKSUM_ERR         ((uint32)1U << 12U)
#define UART_LIN_PID_ERR              ((uint32)1U << 11U)
#define UART_LIN_SYNC_FIELD_ERR       ((uint32)1U << 10U)
#define UART_LIN_ASYNC_INT_FLAG       ((uint32)1U << 9U)
...
static const uint32 uartInterruptMaskTable[]=
{
...
static const uint32 uartInterruptStatusTable[]=
{
...
static const uint32 uartLineStatusTable[]=
{
 ...
 */
/*PRQA S 0303 ++*/
static Reg_Uart_BfType * const uartRegPtr[UART_NUM] = 
{
    (Reg_Uart_BfType *)UART0_BASE_ADDR, 
    (Reg_Uart_BfType *)UART1_BASE_ADDR, 
    (Reg_Uart_BfType *)UART2_BASE_ADDR, 
    (Reg_Uart_BfType *)UART3_BASE_ADDR, 
    (Reg_Uart_BfType *)UART4_BASE_ADDR, 
    (Reg_Uart_BfType *)UART5_BASE_ADDR, 
};

static Reg_Uart_WType * const uartRegWPtr[UART_NUM] = 
{
 ...
 *  @{
 */
static uint32 uartLineStatusBuf[UART_NUM] = 
{
    0U,0U,0U,0U,0U,0U
};
/* FIFO Control register buff */
static uint32 uartFifoControlBuf[UART_NUM] = {0U,0U,0U,0U,0U,0U};
...
static void UART_IntHandler(UART_ID_t uartId)
{
    ...
    Reg_Uart_BfType * UARTx = (Reg_Uart_BfType *)(uartRegPtr[uartId]);
    Reg_Uart_WType * UARTxw = (Reg_Uart_WType *)(uartRegWPtr[uartId]);
    ...
    if(UARTx->UART_DLH_IER.UART_IER.HEADER_DONE_INT_EN == 1U)
    {
        ...
        {
            if(uartIsrCb[uartId][UART_INT_HEADER_DONE] != NULL_PTR)
            {
    ...
    if(UARTx->UART_DLH_IER.UART_IER.RSP_DONE_INT_EN == 1U)
    {
        ...
        {
            if(uartIsrCb[uartId][UART_INT_RSP_DONE] != NULL_PTR)
            {
                /* call the callback function */
    ...
    if(UARTx->UART_DLH_IER.UART_IER.ELSI == 1U)
    {
        ...
        {
            if(uartIsrCb[uartId][UART_INT_LSI] != NULL_PTR)
            {
                /* call the callback function */
    ...
    if( UARTx->UART_DLH_IER.UART_IER.ASYNC_INT_EN == 1U)
    {
        ...
        {
            if(uartIsrCb[uartId][UART_INT_ASYNC] != NULL_PTR)
            {
                /* call the callback function */
    ...
    switch(intId)
    {
        case UART_INTSTA_IID_RBFI:
            if(uartIsrCb[uartId][UART_INT_RBFI] != NULL_PTR)
            {
                /* call the callback function */
        ...
        case UART_INTSTA_IID_TBEI:
            if(uartIsrCb[uartId][UART_INT_TBEI] != NULL_PTR)
            {
                /* call the callback function */
        ...
        case UART_INTSTA_IID_BYDET:  
            ...
            if(uartIsrCb[uartId][UART_INT_BYDET] != NULL_PTR)
            {
                /* call the callback function */
        ...
        case UART_INTSTA_IID_RCVRTO:
            if(uartIsrCb[uartId][UART_INT_RCVRTO] != NULL_PTR)
            {
                /* call the callback function */
        ...
        case UART_INTSTA_IID_MODEM:
            ...
            if(uartIsrCb[uartId][UART_INT_MODEM] != NULL_PTR)
            {
                /* call the callback function */
...
void UART_Init(UART_ID_t uartId, const UART_Config_t* uartConfig)
{
    ...
    uint32 clkFreq = CLK_GetModuleClkFreq(uartClkId, uartConfig->oscFreq);
#endif
    uint32 clkFreq = (uint32)uartConfig->oscFreq;       /*!<Added in order to adapt to mcal */
    Reg_Uart_BfType * UARTx = (Reg_Uart_BfType *)(uartRegPtr[uartId]);
  ...
  /************Configure the Baud Rate*********************************/
    ...
    {
        UARTx->UART_LCR.DLS = (uint32)uartConfig->dataBits ;
    }
    /* Set UART  stop bit */
    UARTx->UART_LCR.STOP = (uint32)uartConfig->stopBits;

    /***********Configure auto flow  *******************/
    UARTx->UART_AFCR.AFCE = (uint32)uartConfig->autoFlowControl;
...
ResultStatus_t UART_WaitBusyClear(UART_ID_t uartId, uint32 cycle)
{
    ...
    volatile uint32 localcount = 0;
    
    while(localcount < cycle)
    {
        if((uint32)RESET == UARTx->UART_USR.BUSY)
        {
            ret = SUCC;
            break;
        }
...
void UART_RtsEnable(UART_ID_t uartId, ControlState_t newState)
{
    ...
    UARTx->UART_AFCR.RTS = (uint32)newState;
...
void UART_SetLoopBackMode(UART_ID_t uartId, ControlState_t newState)
{
    ...
    UARTx->UART_AFCR.LB = (uint32)newState;
...
ResultStatus_t UART_SendBreak(UART_ID_t uartId, uint8 len)
{
    ...
    {
        UARTx->UART_LCR.LBKM = (uint32)len - 0xBU;
...
uint8 UART_ReceiveByte(UART_ID_t uartId)
{
    ...
    /* Receive data */
    return (uint8)(UARTx->UART_RBR_THR_DLL.UART_RBR.RBR_LSB);
}
...
void UART_ReceiveBytes(UART_ID_t uartId,uint32 length, uint8 data[])
{
    ...
    for(uint32 count = 0U; count <length; count++)
    {
        data[count] = (uint8)UARTx->UART_RBR_THR_DLL.UART_RBR.RBR_LSB;
    }
}
...
void UART_EmptyRxFifo(UART_ID_t uartId)
{
    ...
    while(((uint32)UART_GetLineStatus(uartId, UART_LINESTA_DR) & 0x1U) != 0U)
    {
        dummyData = (uint8)UARTx->UART_RBR_THR_DLL.UART_RBR.RBR_LSB;
    }
}
...
{    
    ...
    if((uint32)SET == UARTx->UART_LCR_EXT.DLS_E)
    {   /* Receive mode 0 */
...
uint16 UART_9BitsM1RxAddr(UART_ID_t uartId)
{
    ...
    regData = (uint32)UARTxw->UART_RBR_THR_DLL.UART_RBR;
    rx_addr = (uint8)UARTx->UART_RAR.RAR;
...
void UART_SendByte(UART_ID_t uartId, uint8 data)
{
    Reg_Uart_WType * UARTxw = (Reg_Uart_WType *)(uartRegWPtr[uartId]);
    /* Send data */
    UARTxw->UART_RBR_THR_DLL.UART_THR = (uint32)data;
}
...
void UART_9BitsM0SetAddr(UART_ID_t uartId, uint8 addr)
{
    ...
    if(0x1U == UARTx->UART_LCR_EXT.DLS_E)
    {
        ...
        UARTx->UART_TAR.TAR = (uint32)addr;
    }
}
...
void UART_9BitsM1TxData(UART_ID_t uartId, uint16 data)
{
    ...
    if((uint32)SET == UARTx->UART_LCR_EXT.DLS_E)
    {
        ...
        /* Send data */
        UARTxw->UART_RBR_THR_DLL.UART_THR = ((uint32)data) & 0x1FFU;
    }
}
...
void UART_DebugCmd(UART_ID_t uartId, ControlState_t Cmd)
{
    ...
    UARTx->UART_LCR.DEBUG_EN = (uint32)Cmd;
}
...
void UART_IdleDetectConfig(UART_ID_t uartId,UART_IdleDetectConfig_t *config)
{
    ...
    UARTx->UART_LCR.IDLE_DET_LENGTH = (uint32)config->len;
    UARTx->UART_LCR.IDLE_DET_EN = (uint32)config->Cmd;
}
...
void UART_LinConfig(UART_ID_t uartId, const UART_LinConfig_t *config)
{
    ...
    uint32 integerDiv, fractionalDiv;
/*
    CLK_Module_t clk[] = {CLK_UART0,CLK_UART1,CLK_UART2,CLK_UART3,CLK_UART4,CLK_UART5};
    CLK_Module_t uartClkId = clk[uartId];
*/
    uint32 tmp;
    ...
    tmp = (uint32)config->oscFreq;              /*!<Added in order to adapt to mcal */
    ...
    UARTx->UART_LIN_CTL.MASTER_MODE = (uint32)config->mode;

    /* auto sync */
    UARTx->UART_LCR.AUTO_SYNC_EN = (uint32)config->autoSyncEn;
 
    /* ID filter enable */
    UARTx->UART_LIN_PID_FILTER_CTRL.LIN_FILTER_EN = (uint32)config->idFilterEn;
                                           ...
                                           const UART_LinHeader_t *header)
{
    ...
    uint32 breakLen;
    uint32 rsp_op_start = UARTx->UART_LIN_CTL.RSP_OP_START;
    uint32 header_op_start = UARTx->UART_LIN_CTL.HEADER_OP_START;
                                    ...
                                    const UART_LinResponse_t *rsp)
{
    ...
    else
    {
        ...
        UARTx->UART_LIN_CHECKSUM.CHECKSUM_TYPE = (uint32)rsp->checkType;

        for(uint32 count = 0U; count < rsp->len; count++)
        {
                                            ...
                                            uint32 len)
{
    ...
    else
    {
        ...
        UARTx->UART_LIN_CHECKSUM.CHECKSUM_TYPE = (uint32)check;
...
void UART_LinReadResponse(UART_ID_t uartId, uint32 len, uint8 data[])
{
    ...
    for(uint32 count = 0U; count < len; count++)
    {
        data[count] = (uint8)UARTx->UART_RBR_THR_DLL.UART_RBR.RBR_LSB;
    }
}
...
uint32 UART_GetAllLineStatus(UART_ID_t uartId)
{
    ...
    ret = (uint32)UARTxw->UART_LSR & (uint32)0xFFFF;
                                      ...
                                      uint8 idFilterValue)
{
    ...
    else
    {
        ...
        *idFilterP[idInd/4U] = regTmp | ((uint32)idFilterValue << tmp);
                                           ...
                                           uint8 idFilterMask)
{
    ...
    UARTx->UART_LIN_PID_FILTER_CTRL.FID_MASK |= ((uint32)1U << 16U);
}
                                       ...
                                       const uint8 idValues[])
{
    ...
    {
        for(uint8 index = 0U; index < idNum; index++)
        {
            tmp = (index % 4UL) * 8UL;
            regTmp = *idFilterP[index/4UL] & ~(0x3FUL << tmp);
            *idFilterP[index/4UL] = regTmp | (uint8)(idValues[index] << tmp);
 ...
 *
 */
void Uart_Drv_1_IrqHandler(void)
{
 ...
 *
 */
void Uart_Drv_2_IrqHandler(void)
{
 ...
 *
 */
void Uart_Drv_3_IrqHandler(void)
{
 ...
 *
 */
void Uart_Drv_4_IrqHandler(void)
{
 ...
 *
 */
void Uart_Drv_5_IrqHandler(void)
{
