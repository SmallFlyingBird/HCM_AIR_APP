...
#define CANIF_C_SW_PATCH_VERSION 5u
/*******************************************************************************
**                       Version  Check                                       **
*******************************************************************************/
...
#define CANIF_METADATATOCANID(CanId, MetaDataPtr)                                  \
    CanId =                                                                        \
        ((CanIf_IdType)(MetaDataPtr[0u]) | ((CanIf_IdType)(MetaDataPtr[1u]) << 8u) \
...
#define CANIF_METADATATOCANID(CanId, MetaDataPtr)                                            \
    CanId =                                                                                  \
        (((CanIf_IdType)(MetaDataPtr[0u]) << 24u) | ((CanIf_IdType)(MetaDataPtr[1u]) << 16u) \
...
#endif
#if (STD_ON == CANIF_PUBLIC_PN_SUPPORT)
                if (CANIF_CONTROLLER(canLoop).CanIfPnFilter)
                {
...
CanIf_Transmit(PduIdType CanIfTxSduId, P2CONST(PduInfoType, AUTOMATIC, CANIF_APPL_DATA) PduInfoPtr)
{
    ...
    if (detNoErr && (txPduConfigPtr->CanIfTxPduRef->TxMetaDataEnable) && (NULL_PTR == PduInfoPtr->MetaDataPtr))
    {
        (void)Det_ReportError(CANIF_MODULE_ID, CANIF_INSTANCE_ID, CANIF_TRANSMIT_ID, CANIF_E_PARAM_POINTER);
        detNoErr = FALSE;
    }
...
#if (0u < CANIF_RXBUFFER_NUMBER)
    {
            ...
            && ((CANIF_OFFLINE != CanIf_PduMode[controllerId])))
        {
            ...
            if ((NULL_PTR != rxPduBuffer) && (0u != rxPduBuffer->length))
            {
                ...
                (void)memcpy(CanIfRxInfoPtr->SduDataPtr, rxPduBuffer->data, CanIfRxInfoPtr->SduLength);
#if (STD_ON == CANIF_META_DATA_SUPPORT)
                if (rxPduConfigPtr->CanIfRxPduRef->RxMetaDataEnable)
                {
...
#if (STD_ON == CANIF_PUBLIC_PN_SUPPORT)
                if (CANIF_CONTROLLER(ControllerId).CanIfPnFilter)
                {
...
CanIf_CheckValidation(EcuM_WakeupSourceType WakeupSource)
{
...
#endif /*STD_ON == CANIF_PUBLIC_DEV_ERROR_DETECT*/
    {
        ...
        EcuM_WakeupSourceType wakeupSource;
        const CanIf_ControllerCfgType* ctrlCfgPtr = &CANIF_CONTROLLER(0);

        /*check all can controllers to find the wake-up source*/
        for (controllerIndex = 0u; controllerIndex < CANIF_CANCONTROLLER_NUMBER; controllerIndex++)
        {
            wakeupSource = ctrlCfgPtr->CanIfCtrlCanCtrlRef->CanIfWakeUpSource;
...
/* PRQA S 1532 -- */ /* MISRA Rule 8.7 */
{
    ...
    uint8 controllerId;
    const CanIf_ControllerCfgType* ctrlCfgPtr;
    ...
    if (detNoErr && (CanTxPduId >= CANIF_TXPDU_NUMBER))
    {
    ...
    || (STD_ON == CANIF_PUBLIC_PN_SUPPORT))
        ...
        ctrlCfgPtr = &CANIF_CONTROLLER(controllerId);
#endif /*(CANIF_TXBUFFER_NUMBER > 0u) || (STD_ON == CANIF_PUBLIC_TX_CONFIRM_POLLING_SUPPORT) ||(STD_ON == \
          CANIF_PUBLIC_PN_SUPPORT)*/
#if (CANIF_TXBUFFER_NUMBER > 0u)
        ...
        if (txBufIndex < CANIF_TXBUFFER_NUMBER)
        {
            ...
            for (index = 0u; index < txBufSize; index++)
            {
                if (txPduBufferPtr[index].used)
                {
                    ...
                    {
                        canPdu.id = canIdValue;
                        txIndex = index;
                    }
                    else
                    {
                        if (canPdu.id > canIdValue)
                        {
                            canPdu.id = canIdValue;
                            txIndex = index;
                        }
            ...
            if (CANIF_UNUSED_UINT8 != txIndex)
            {
                ...
                canDriver = ctrlCfgPtr->CanDriverId;
...
#include "CanIf_MemMap.h"
#if (1)
extern uint32 dmoni_rxcanid;
extern uint32 dmoni_rxnumber;
extern uint8  dmoni_rxcanfr[8];
#endif
FUNC(void, CANIF_CODE)
/* PRQA S 1532 ++ */ /* MISRA Rule 8.7 */
CanIf_RxIndication(
...
/* PRQA S 1532 -- */ /* MISRA Rule 8.7 */
{
    ...
    /* check parameter valid */
    if (detNoErr && ((NULL_PTR == Mailbox) || (NULL_PTR == PduInfoPtr)))
    {
    ...
    {
#endif /*STD_ON == CANIF_PUBLIC_DEV_ERROR_DETECT*/
        canId = (CanIf_IdType)Mailbox->CanId;
#if (1)
        if (dmoni_rxcanid != 0)
        {
            if (canId == dmoni_rxcanid)
            {
                dmoni_rxcanfr[0] = PduInfoPtr->SduDataPtr[0];
                dmoni_rxcanfr[1] = PduInfoPtr->SduDataPtr[1];
                dmoni_rxcanfr[2] = PduInfoPtr->SduDataPtr[2];
                dmoni_rxcanfr[3] = PduInfoPtr->SduDataPtr[3];
                dmoni_rxcanfr[4] = PduInfoPtr->SduDataPtr[4];
                dmoni_rxcanfr[5] = PduInfoPtr->SduDataPtr[5];
                dmoni_rxcanfr[6] = PduInfoPtr->SduDataPtr[6];
                dmoni_rxcanfr[7] = PduInfoPtr->SduDataPtr[7];
                dmoni_rxnumber++;
            }
        }
#endif
        ...
        {
            if (0x80000000u != (canId & 0xa0000000u))
            {
        ...
        {
            if (0x00000000u != (canId & 0xbffff800u))
            {
                (void)Det_ReportError(CANIF_MODULE_ID, CANIF_INSTANCE_ID, CANIF_RXINDICATION_ID, CANIF_E_PARAM_CANID);
                detNoErr = FALSE;
            }
...
#endif /*STD_ON == CANIF_PUBLIC_DEV_ERROR_DETECT*/
    {
        if (E_OK == CanIf_CanIdRangCheck(Mailbox->ControllerId, Mailbox->Hoh, canId, &rxPduIndex))
        {
/* DLC check enabled */
...
static FUNC(Std_ReturnType, CANIF_CODE) CanIf_SetControllStarted(uint8 Controller)
{
    ...
    uint8 controllerId = controllerPtr->CanIfCtrlCanCtrlRef->CanCtrlId;
...
/*The controllerId is CanIf's controller index*/
    ...
    if (findCanIfHrh)
    {
        ...
        if (CANIF_BASIC_CAN == hrhPtr->CanIfHrhIdSymRef->CanIfHrhType)
        {
            Can_IdType canIdMasked = canId & CANIF_EXTENDED_CANID_MAX;
            /* do HRH software filter*/
            for (cnt = 0u; cnt < hrhCanIdRangeNum; cnt++)
            {
    ...
    if (txBufIndex < CANIF_TXBUFFER_NUMBER)
    {
        ...
        for (index = 0u; (index < txBufferSize) && (E_NOT_OK == result); index++)
        {
            if (txPduBufferPtr[index].TxPduIndex == pduPtr->swPduHandle)
            {
...
#endif /*STD_ON == CANIF_TRIGGER_TRANSMIT_SUPPORT*/
                {
                    ...
                    (void)memcpy(txPduBufferPtr[index].data, pduPtr->sdu, (PduLengthType)pduPtr->length);
                }
                result = E_OK;
            }
        ...
        if ((E_NOT_OK == result) && (CANIF_UNUSED_UINT8 != idleBufferIndex))
        {
...
#endif /*STD_ON == CANIF_TRIGGER_TRANSMIT_SUPPORT*/
            {
                ...
                (void)memcpy(txPduBufferPtr[idleBufferIndex].data, pduPtr->sdu, (PduLengthType)pduPtr->length);
            }
            result = E_OK;
        }
    }
...
#if (STD_ON == CANIF_META_DATA_SUPPORT)
        if (txPduConfigPtr->CanIfTxPduRef->TxMetaDataEnable)
        {
...
/* rx status notify enabled */
    ...
    if (NULL_PTR != rxPduBuffer)
    {
        ...
        /* save sdu */
        (void)memcpy(rxPduBuffer->data, PduInfoPtr->SduDataPtr, PduInfoPtr->SduLength);
#if (STD_ON == CANIF_META_DATA_SUPPORT)
    ...
    if (NULL_PTR != rxPduBuffer)
    {
        ...
        /* save sdu */
        (void)memcpy(rxPduBuffer->data, pduInfo.SduDataPtr, pduInfo.SduLength);
        if (metaDataEnable)
        {
