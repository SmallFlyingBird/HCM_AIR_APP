...
*/
/* MR12 RULE 18.4 VIOLATION: C90 rule, retired in C99.
...
__STATIC_INLINE void ARM_MPU_Load(ARM_MPU_Region_t const *table, uint32 cnt)
{
    ...
    ARM_MPU_Region_t const *tableTemp;
    uint32 cntTemp = cnt;
    while (cntTemp > MPU_TYPE_RALIASES)
    {
        ARM_MPU_OrderedMemcpy(&(MPU->RBAR), &(table->RBAR), MPU_TYPE_RALIASES * rowWordSize);
        tableTemp += MPU_TYPE_RALIASES;  /* PRQA S 0488 */
        cntTemp += MPU_TYPE_RALIASES;
    }
    ARM_MPU_OrderedMemcpy(&(MPU->RBAR), &(tableTemp->RBAR), cntTemp * rowWordSize);
}
