...
uint8 Sha256Calc_calculate(Sha256Calc* t, const uint8* dp, uint32 dl)
{
	...
	*/
	z1 = ( uint32 )(( t->ByteNumLo & 0x7fffffffuL ) + dl);
	x = (( t->ByteNumLo >> 31u ) & 0x01uL ) + (( z1 >> 31u ) & 0x01uL );
	z1 = ( z1 & 0x7fffffffuL ) | ( x << 31u );
	y = t->ByteNumHi + ( x >> 1u );
	if( y > 0x1fffffffuL )
	{
	...
	else
	{
		...
		*/
		while( w > 0u )
		{
			...
			while( x < 16u )
			{
				t->Padding[ x ] = ( t->Padding[ x ] & ~( 0x000000ffuL << ( 24u - y ))) | (( dp[ z1 ] &
				0x000000ffuL ) << ( 24u - y ));
				y = y + 8u;
				x += ( y >> 5u );
				y &= 0x1fuL;
		...
		/**
		   Last Blocks Loop
		   z1 is how many bytes used. dl is bigger than z1. -- Also Big endian Order !
		*/
		w = ( uint32 )( dl - z1 );
		while( w > 0u )
		{
			t->DwordBuf[ x ] =
				( t->Padding[ x ] & ~( 0x000000ffuL << ( 24u - y ))) | (( dp[ z1 ] &
				0x000000ffuL ) << ( 24u - y ));
			t->Padding[ x ] =
				( t->Padding[ x ] & ~( 0x000000ffuL << ( 24u - y ))) | (( dp[ z1 ] &
							0x000000ffuL ) << ( 24u - y ));
...
static void Sha256Calc_calcBlock( uint32* dp, uint32* rp )
{
	uint32 a,b,c,d,e1,f,g,h;
	uint32 t0,t1,t2;
	uint32 x;

	/*Security Public Key : 256Bytes,sha256 initial value*/
	const uint32 Sha256Calc_k[64u] =
	{
	...
	for( x = 0; x < 64u; x ++ )
	{
		...
		t1 = ((( e1 >> 6u ) & 0x3ffffffuL ) | ( e1 << 26u )) ^
		     ((( e1 >> 11u ) & 0x1fffffuL ) | ( e1 << 21u )) ^
		     ((( e1 >> 25u ) & 0x7fuL ) | ( e1 << 7u ));
		t0 = ( e1 & f ) ^ ((~e1) & g );
