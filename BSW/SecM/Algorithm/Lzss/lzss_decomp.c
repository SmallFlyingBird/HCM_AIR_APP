...
void Lzss_Decomp(uint8 *Buf, uint32 Length, BIT_BUFF *outBuf)
{
	...
	outBuf->PcCount = 0;
	outBuf->lastreaminbit = 0;
	outBuf->lastreaminbitlength = 0;
...
static void Lzss_ExpandStreamData(BIT_BUFF *inBuf, BIT_BUFF *outBuf, uint32 length)
{
	...
	while ( eosReached == FALSE && bitlength > 0)
	{
		...
		if(inBuf->lastreaminbitlength > 0u)
		{
			...
			if ( (inBuf->lastreaminbit & (1u << (inBuf->lastreaminbitlength - 1u))) != 0u)
			{
				if (inBuf->lastreaminbitlength + bitlength < 9)
				{
					lastreamindata = 0;
					/*Get the remaining bits*/
					for(i = 0; i < (bitlength-1u);i++ )
					{
						lastreamindata += (uint16)1u << i;
					}
					lastreamindata &= (uint16)Lzss_InputBits( inBuf, (uint32)bitlength - 1);
					lastreamindata += (uint16)1u << ((uint8)bitlength -1u);
					outBuf->lastreaminbit = (inBuf->lastreaminbit << bitlength) + lastreamindata;
					/*Get the remaining bits length*/
					outBuf->lastreaminbitlength =(uint8) bitlength + inBuf->lastreaminbitlength;
					break;
				}
			...
			else
			{
				...
				if (inBuf->lastreaminbitlength + bitlength < 15)
				{
					lastreamindata = 0;
					/*Get the remaining bits*/
					...
					lastreamindata &= (uint16)Lzss_InputBits( inBuf, (uint32)bitlength);
					outBuf->lastreaminbit = (inBuf->lastreaminbit << bitlength) + lastreamindata;
					/*Get the remaining bits length*/
					outBuf->lastreaminbitlength =(uint8) bitlength + inBuf->lastreaminbitlength;
					break;
