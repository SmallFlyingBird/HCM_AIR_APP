 ...
 * @return      NA
 */
COMM_LOCAL void ComM_ChReqModeMainHandle(uintx chIdx, const uintx* reqIdexPtr)
{
...
COMM_LOCAL void ComM_ChStateTransitionToNoCom(uintx chIdx)
{
    ...
    do
    {
        ...
        case COMM_SPEC_NOCOM_REQ_PENDING:
             ...
             */
            if ((chVarPtr->reqCnt == 0u) && !COMM_CH_IS_MASK(chVarPtr, COMM_FUN_PASSIVE_WAKEUP))
            {
                ComM_ChBeh_EntryNoCom(chIdx);
            }
            break;
...
#if (COMM_MODE_LIMITATION_ENABLED == STD_ON)
        if (COMM_CH_CFG_IS(chCfgPtr, COMM_NO_COM_INHIBITION))
        {
...
FUNC(void, COMM_CODE) ComM_ChProcessTimer(uint8 chIdx) /* PRQA S 1532 */ /* MISRA Rule 8.7 */
{
    ...
    ComM_ChVarType* chVarPtr = &ComM_ChVarTable[chIdx];
#endif /* (COMM_USER_MODESWITCH_NOTIFY == STD_ON) || defined(COMM_NM_VARIANT_LIGHT) || defined(COMM_NM_VARIANT_NONE) \
        */
...
void ComM_ChLimitWakeUp(uintx chIdx, boolean stu)
{
    ...
    else
    {
        ...
        /* The "highest win" strategy would apply immediately (SWS_ComM_00302) */
        const ComM_ChannelConfigType* chCfgPtr = &ComM_ChannelConfigPtr[chIdx];
        for (const ComM_UserHandleType* userPtr = chCfgPtr->userBegin; userPtr < chCfgPtr->userEnd; ++userPtr)
        {
            if (ComM_UserReqMode[*userPtr] == COMM_FULL_COMMUNICATION)
            {
                ComM_HandleUserReqest(*userPtr, COMM_FULL_COMMUNICATION);
            }
        }
    }
    SchM_Exit_ComM_COMM_EXCLUSIVE_AREA_0();
