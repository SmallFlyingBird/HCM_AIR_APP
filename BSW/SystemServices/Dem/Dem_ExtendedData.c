        ...
        for (uint8 index = 0; index < RefNum; index++)
        {
            if (*pExtIndex != DEM_EXTENDED_DATA_RECORD_INVALID)
            {
                ...
                if (pExtRecord->DemExtendedDataRecordNumber == RecordNum)
                {
                    ...
                    if (0x00u != DEM_BITS_ISSET(pEntry->ExtStatus, index))
                    {
                        uint8 DataSize = pExtRecord->DataSize;
                        ret = DEM_RECORD_WRONG_BUFFERSIZE;
                        if ((uint16)(0 + DataSize) <= (*BufSize)) /* PRQA S 4391 */ /* MISRA Rule 10.8 */
                        {
#if (DEM_DATA_ELEMENT_CLASS_NUM > 0u)
                            ...
                            {
                                Dem_MemCopy(&pBuffer[0], &ExtData[offset], DataSize);
                            }
                            else
                            {
                                /* get inter data now */
                                pDemDataElementClass->DemDataElementReadFnc(pBuffer);
                            }
#endif /* DEM_DATA_ELEMENT_CLASS_NUM > 0u */
                            rtsize = DataSize;
                            ret = E_OK;
            ...
            offset += DemExtendedDataRecordClass[*pExtIndex].DataSize;
            pExtIndex++;
        }
        *BufSize = rtsize;
    }
            ...
            for (uint8 iloop = 0; iloop < RefNum; iloop++)
            {
                ...
                if (pExtIndex != DEM_EXTENDED_DATA_RECORD_INVALID)
                {
                    ...
                    if ((RecordTrigger == Trigger)
                        ...
                        && ((Update == DEM_UPDATE_RECORD_YES) || (0x00u == DEM_BITS_ISSET(pEntry->ExtStatus, iloop))))
                    {
