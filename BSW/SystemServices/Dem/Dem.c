...
#define DEM_START_SEC_VAR_INIT_8
#include "Dem_MemMap.h"
DEM_LOCAL VAR(boolean, AUTOMATIC) DemCheckEnableCondition = FALSE;
#define DEM_STOP_SEC_VAR_INIT_8
#include "Dem_MemMap.h"
#endif
...
#define DEM_START_SEC_VAR_INIT_8
#include "Dem_MemMap.h"
DEM_LOCAL VAR(boolean, AUTOMATIC) DemCheckStorageCondition = FALSE;
#define DEM_STOP_SEC_VAR_INIT_8
#include "Dem_MemMap.h"
#endif
...
#define DEM_START_SEC_VAR_INIT_8
#include "Dem_MemMap.h"
DEM_LOCAL VAR(boolean, AUTOMATIC) FirstStartUp = FALSE; /* PRQA S 3218*/ /* MISRA Rule 8.9 */
#define DEM_STOP_SEC_VAR_INIT_8
#include "Dem_MemMap.h"
#endif
/*******************************************************************************
...
FUNC(void, DEM_CODE) Dem_ReportErrorStatus(Dem_EventIdType EventId, Dem_EventStatusType EventStatus)
{
...
#endif /* STD_ON == DEM_DEV_ERROR_DETECT */
    {
        ...
        else if ((DemInitState == DEM_STATE_PRE_INIT) && (pEventCfg->DemReportBehavior != REPORT_BEFORE_INIT))
        {
            DEM_DET_REPORT(DEM_SID_REPORTERRORSTATUS, DEM_E_WRONG_CONFIGURATION);
        }
        else if (
...
Dem_SetOperationCycleState(uint8 OperationCycleId, Dem_OperationCycleStateType CycleState)
{
...
#endif /* STD_ON == DEM_DEV_ERROR_DETECT */
    {
        if (CycleState == DEM_CYCLE_STATE_START)
        {
            ...
            if (0x00u
                != DEM_BITS_ISSET(DemOperationCycleStatus, OperationCycleId)) /* PRQA S 2985 */ /* MISRA Rule 2.2 */
            {
...
#endif /* STD_ON == DEM_DEV_ERROR_DETECT */
    {
        DemComponentStatusType* pDemComponentStatus = &DemComponentStatus[Dem_GetEventInternalId(ComponentId)];
        /*SWS_Dem_01134 */
...
#ifdef ADD_BY_MHL
Std_ReturnType Dem_DebounceStatusHandle(Dem_EventIdType EventId,Dem_EventStatusType EventStatus)
{
	uint16 IntId;
               ...
               &&( (pEventCfg->AlgorithmIndex) < DEM_DEBOUNCE_COUNTER_BASED_EVENT_NUM ) )
            {
            	...
            	switch(EventStatus)
            	{
					case DEM_EVENT_STATUS_PREPASSED:
						if(pCounter->InternalDebounceCounter>pCfgCounter->DemDebounceCounterPassedThreshold)
						{
							ret=E_NOT_OK;
						}
...
DEM_LOCAL FUNC(void, DEM_CODE) Dem_GetEVCOMBEntry(Dem_EventMemEntryType* pEntry, const uint8* pMemDest, uint16 dtcRef)
{
    const Dem_EventParameterType* pDemEventParameter = DemPbCfgPtr->DemEventParameter;
...
#if (DEM_EVENT_COMBINATION_SUPPORT == DEM_EVCOMB_ONSTORAGE)
            Dem_GetEVCOMBEntry(pEntry, pMemDest, dtcRef);
#else
...
/* PRQA S 0624-- */ /* MISRA Rule 8.3 */
{
...
#endif /* STD_ON == DEM_DEV_ERROR_DETECT */
    {
        ...
        {
            if (DemClearDTCLock == DEM_CLEAR_NOT_LOCK)
            {
...
#endif /* DEM_TRIGGER_FIM_REPORTS == STD_ON */
    Dem_ProcessEventPtr Dem_ProcessEventFuncMatrix[] = {
...
#if (DEM_TRIGGER_FIM_REPORTS == STD_ON)
    uint8 NewStatus = pEvent->UdsStatus;
