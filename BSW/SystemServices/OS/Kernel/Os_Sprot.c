...
#include "Os_MemMap.h"
P2VAR(TrustedFunctionIndexType, AUTOMATIC, OS_VAR)Os_TrustedFuncNestQueue = NULL_PTR; /* PRQA S 3432 */ /* MISRA Rule 20.7 */
...
FUNC(void, OS_CODE) Os_InitTrustedFunction(void)
{
    ...
    case 3:
        ...
        for (i = 0u; i < CFG_TRUSTED_SYSTEM_SERVICE_MAX_CORE3; i++)
        {
            Os_TrustedFuncNestQueue[i] = OS_APPLICATION_INVALID;
        }
        break;
    ...
    case 4:
        ...
        for (i = 0u; i < CFG_TRUSTED_SYSTEM_SERVICE_MAX_CORE4; i++)
        {
            Os_TrustedFuncNestQueue[i] = OS_APPLICATION_INVALID;
        }
        break;
    ...
    case 5:
        ...
        for (i = 0u; i < CFG_TRUSTED_SYSTEM_SERVICE_MAX_CORE5; i++)
        {
            Os_TrustedFuncNestQueue[i] = OS_APPLICATION_INVALID;
        }
        break;
 ...
 * one OS-Application to another are limited to the same core.*/
    ...
    OS_CALLTRUSTEDFUN_EXIT(); /* PRQA S 1842 */ /* MISRA Rule 10.4 */
    return OsReturnErr;
}
...
)
{
    ...
    else
    {
        ...
        {
            if ((Os_TaskStack[TaskID].stackTop >= Address) && (Os_TaskStack[TaskID].stackBottom <= Address))
            {
        ...
        {
            if ((Os_ISR2Stack[Isr2ID].stackTop  >= Address) && (Os_ISR2Stack[Isr2ID].stackBottom <= Address))
            {
                Access = OSMEMORY_SET_WRITEABLE(Access);
            }
            else
