...
{
    buffer[0u]  = (uint8)(data >> 24u) & 0xFFu;
    buffer[1u]  = (uint8)(data >> 16u) & 0xFFu;
    buffer[2u]  = (uint8)(data >> 8u)  & 0xFFu;
    buffer[3u]  = (uint8)(data)        & 0xFFu;
...
sint32 DP_Util_U8Op_CompareImmConstantTime(const uint8 *op1,uint32 op1Length,uint8 immediate)
{
    ...
    if (op1Length > 0U)
    {
        ...
        cmpValue |= 0U - cmpValue;
        cmpValue >>= 7;
        mask = 0U - (uint32) cmpValue;
