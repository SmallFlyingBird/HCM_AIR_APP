...
typedef void (*Dcm_PendingFct)(const uint8 errorCode, const Dcm_BuffType* rxBuff, Dcm_BuffType* txBuff);
...
typedef uint8 (*Dcm_ReadDataFct)(uint8* readData, uint16* readLength);
...
typedef uint8 (*Dcm_GenerateSeedFct)(uint8 *randomMsgOutput, uint32 randomMsgOutputLength);

/** compare key function type */
typedef uint8 (*Dcm_CompareKeyFct)(uint8* signature, uint32 signatureLength, uint8* randomMsgInput, uint32 randomMsgInputLength);
...
typedef void (*Dcm_RoutineControlCalloutFct)(const uint8 errorCode, const Dcm_BuffType* rxBuff, Dcm_BuffType* txBuff);
