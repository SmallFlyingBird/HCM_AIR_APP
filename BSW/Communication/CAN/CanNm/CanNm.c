...
#define CANNM_C_SW_PATCH_VERSION         22u
/*******************************************************************************
**                      Include Section                                       **
*******************************************************************************/
#include "CanNm.h"
...
/* pn fill PNI bit */
            ...
            if (CanNm_CfgPtr->ChannelConfig[chIndex].PnEnabled)
            {
                uint8 pnIdx;
...
/* clang-format on */
{
    ...
    else
    {
        ...
        SchM_Enter_CanNm_Network(chIndex);
        (void)memcpy(userDtaPtr, nmUserDataPtr, userDataLength);
        SchM_Exit_CanNm_Network(chIndex);
...
CanNm_GetUserData(NetworkHandleType nmChannelHandle, uint8* nmUserDataPtr) /* PRQA S 1532 */ /* MISRA Rule 8.7 */
{
    ...
    else
    {
        ...
        SchM_Enter_CanNm_Network(chIndex);
        (void)memcpy(nmUserDataPtr, userDtaPtr, userDataLength);
        SchM_Exit_CanNm_Network(chIndex);
...
#endif /* (CANNM_COM_USERDATA_SUPPORT == STD_ON) || (CANNM_GLOBAL_PN_SUPPORT == STD_ON) */
...
/* PRQA S 1532 -- */ /* MISRA Rule 8.7 */
{
    ...
    else
    {
        ...
        /* @req SWS_CanNm_00132 */
        if (CanNm_GetNodeIdEnabled(chIndex))
        {
            SchM_Enter_CanNm_Network(chIndex);
...
Std_ReturnType CanNm_RepeatMessageRequest(NetworkHandleType nmChannelHandle) /* PRQA S 1532 */ /* MISRA Rule 8.7 */
{
    ...
    {
        if (CanNm_GetNodeDetectionEnabled(chIndex))
        {
            CanNm_InnerChannelType* chRTPtr = &CanNm_GetChannelRTData(chIndex);
...
/* PRQA S 1532 -- */ /* MISRA Rule 8.7 */
{
    ...
    else
    {
        ...
        if (FALSE
#if CANNM_NODE_ID_ENABLED == STD_ON
            || CanNm_GetNodeIdEnabled(chIndex)
#endif /* CANNM_NODE_ID_ENABLED == STD_ON */
#if CANNM_NODE_DETECTION_ENABLED == STD_ON
            || CanNm_GetNodeDetectionEnabled(chIndex)
#endif /* CANNM_NODE_DETECTION_ENABLED == STD_ON */
        )
#endif /* CANNM_USER_DATA_ENABLED == STD_OFF */
        {
            CanNm_InnerChannelType* chRTPtr = &CanNm_GetChannelRTData(chIndex);
            SchM_Enter_CanNm_Network(chIndex);
            (void)memcpy(nmPduDataPtr, chRTPtr->rxPduData, CANNM_DEFAULT_PDU_LENGTH);
            SchM_Exit_CanNm_Network(chIndex);
            ret = E_OK;
        }
    }
...
/* PRQA S 1532 -- */ /* MISRA Rule 8.7 */
{
...
#endif /* STD_ON == CANNM_DEV_ERROR_DETECT */
    {
        ...
        else
        {
            ...
            if (CanNm_FindChannelIndex(nmChannelHandle, &chIndex))
            {
                SchM_Enter_CanNm_Network(chIndex);
...
/* @req SWS_CanNm_00337, @req SWS_CanNm_00339*/
...
#endif
        {
            validNmMsgFlag = TRUE;
            /* @req SWS_CanNm_00035 */
            (void)memcpy(chRTPtr->rxPduData, PduInfoPtr->SduDataPtr, (uint16)PduInfoPtr->SduLength);
    ...
    if (detNoErr)
    {
        ...
        if (PduInfoPtr->SduLength >= CANNM_DEFAULT_PDU_LENGTH)
        {
            ...
            if (chRTPtr != NULL_PTR) /* && (chCfgPtr != NULL_PTR) */
            {
                ...
                {
                    /* @req SWS_CanNm_00351 */
                    (void)memcpy(PduInfoPtr->SduDataPtr, chRTPtr->txPduData, CANNM_DEFAULT_PDU_LENGTH);
                }
            }
        }
    }
...
{
    if (CanNm_InitStatus == CANNM_INIT_SUCCESS)
    {
...
static FUNC(void, CANNM_CODE) CanNm_RxCarWakeUpHadle(uint8 chIndex)
{
    ...
    if (CanNm_GetCarWakeUpRxEnabled(chIndex) && ((carWakeupByte & carWakeupMask) > 0u))
    {
        ...
        if (CanNm_GetCarWakeUpFilterEnabled(chIndex))
        {
            /* @req SWS_CanNm_00408 */
        ...
        if ((filterMaskData & bitMask) > 0u)
        {
            ...
            if (0u != (rxPnInfoDta & bitMask))
            {
                ...
                if (chCfgPtr->PnEraCalcEnabled)
                {
                     ...
                     */
                    chRTPtr->pnEraResetTimer[pnIdx] = CanNm_GetPnResetTime();
                }
#endif /* (STD_ON == CANNM_PN_ERA_CALC_ENABLED) */

#if (STD_ON == CANNM_PN_EIRA_CALC_ENABLED)
                /* @req SWS_CanNm_00349 */
                pnEiraResetTimerPtr[pnIdx] = CanNm_GetPnResetTime();
#endif /* (STD_ON == CANNM_PN_EIRA_CALC_ENABLED) */
            }
            pnIdx++;
        }
...
static FUNC(void, CANNM_CODE) CanNm_BusSleepStateHandle(uint8 chIndex)
{
...
#endif /*#if ((STD_ON == CANNM_DEV_ERROR_DETECT) && (STD_ON == CANNM_VER_R19_11))*/
    }
    ...
    {
        if (CanNm_GetChannelConfig(chIndex).ActiveWakeupBitEnabled)
        {
            /* @req SWS_CanNm_00401 */
...
static FUNC(void, CANNM_CODE) CanNm_PrepareBusSleepStateHandle(uint8 chIndex)
{
    ...
    {
        if(Interface_GetKeepAwakeFlag()==1)
        {
            const CanNm_ChannelConfigType* chCfgPtr = &CanNm_GetChannelConfig(chIndex);
...
exit:; /* do notiong */
#endif /* (CANNM_PASSIVE_MODE_ENABLED == STD_OFF) && (CANNM_GLOBAL_PN_SUPPORT == STD_ON) */
}
...
static FUNC(void, CANNM_CODE) CanNm_NormalOperationStateHandle(uint8 chIndex)
{
    ...
    const CanNm_ChannelConfigType* chCfgPtr = &CanNm_GetChannelConfig(chIndex);
#endif /* (STD_ON == CANNM_GLOBAL_PN_SUPPORT) || (STD_ON == CANNM_NODE_DETECTION_ENABLED) */
    CanNm_InnerChannelType* chRTPtr = &CanNm_GetChannelRTData(chIndex);
...
/*the busload reduction is only work in Normal operation mode*/
    ...
    if ((NM_STATE_NORMAL_OPERATION == chRTPtr->canNmState) && (chRTPtr->rxPduExtFlg))
    {
        ...
        if (CanNm_GetBusLoadReduceEnabled(chIndex))
        {
...
#if (STD_ON == CANNM_NODE_DETECTION_ENABLED)
    if (CanNm_GetNodeDetectionEnabled(chIndex) && chRTPtr->repeatMessageRequest && !stateChanged)
    {
        /* @req SWS_CanNm_00112 */
        CanNm_EntryRepeatMessageState(chIndex, FALSE);
        /* @req SWS_CanNm_00113 */
...
#if (STD_ON == CANNM_NODE_DETECTION_ENABLED)
    if (CanNm_GetNodeDetectionEnabled(chIndex) && chRTPtr->rxPduExtFlg && (CANNM_PDU_OFF != chCfgPtr->PduCbvPosition)
        && !stateChanged)
    {
        uint8 cbvPos = (uint8)chCfgPtr->PduCbvPosition;
        uint8 cbvDta = chRTPtr->rxPduData[cbvPos];
...
/* Detection of Remote Sleep Indication Handle */
     ...
     */
    if ((CANNM_PDU_OFF != chCfgPtr->PduCbvPosition) && CanNm_GetNodeDetectionEnabled(chIndex)
        && CanNm_GetRepeatMsgIndEnabled(chIndex))
    {
        uint8 cbvPos = (uint8)chCfgPtr->PduCbvPosition;
        uint8 cbvDta = chRTPtr->rxPduData[cbvPos];
...
#if (STD_ON == CANNM_CAR_WAKEUP_RX_ENABLED)
    if (CanNm_GetCarWakeUpRxEnabled(chIndex))
    {
...
#if ADD_BY_MHL_CANNM
void CanNmResetPncTimerHander(const uint8 *CanSduPtr)
{
    P2CONST(CanNm_PnInfoType, AUTOMATIC, CANNM_APPL_CONST)
    pnInfoPtr = &CanNm_GetPnInfo();
    uint16 *pnEiraResetTimerPtr = CanNm_GetPnEiraResetTimer();
    ...
    for (uint8 chIndex = 0u; chIndex < CANNM_NUMBER_OF_CHANNEL; chIndex++)
    {
        ...
        for (index = 0u; index < (pnInfoPtr->CanNmPnInfoLength * 8u); index++)
        {
            ...
            if ((filterMaskData & bitMask) > 0u)
            {
                rxPnInfoDta = CanSduPtr[(index / 8u + pnInfoPtr->CanNmPnInfoOffset)];
                ...
                if (0u != (rxPnInfoDta & bitMask))
                {
                    ...
                    if (chCfgPtr->PnEraCalcEnabled)
                    {
                         ...
                         * this PN shall be restarted.
                         */
                        chRTPtr->pnEraResetTimer[gpnIdx] = CanNm_GetPnResetTime();
                    }
#endif /* (STD_ON == CANNM_PN_ERA_CALC_ENABLED) */

#if (STD_ON == CANNM_PN_EIRA_CALC_ENABLED)
                    /* @req SWS_CanNm_00349 */
                    pnEiraResetTimerPtr[gpnIdx] = CanNm_GetPnResetTime();
#endif /* (STD_ON == CANNM_PN_EIRA_CALC_ENABLED) */
                }
                gpnIdx++;
            }
