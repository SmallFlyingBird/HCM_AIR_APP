...
#define CANSM_C_SW_PATCH_VERSION         14u
/*******************************************************************************
**                      Include Section                                       **
*******************************************************************************/
#include "CanIf.h"
...
static void CanSM_FullCom_S_No_Bus_Off(uint8 netID, CanSM_NetWorkRunTimeType* networkPtr);
...
*******************************************************************************/
#if CANSM_BUSOFF_HANDLER_MHL_ENABLE
static uint8_t MhlBusOffFLag=0;
#endif
/*******************************************************************************
...
#endif
    {
...
#endif /* CANSM_DEV_ERROR_DETECT == STD_ON */
        {
            ...
            CANSM_FOREACH_NETWORK_CONTROLLER_START(networkIndex, controllerId)
            {
                ...
                ret = CanIf_SetIcomConfiguration(controllerId, ConfigurationId);
                if (ret != E_OK)
                {
                    break;
                }
...
CanSM_StopWakeupSource(NetworkHandleType network) /* PRQA S 1532 */ /* MISRA Rule 8.7 */
{
...
#endif
    {
...
#endif /* CANSM_DEV_ERROR_DETECT == STD_ON */
        {
            ...
            {
#if (STD_ON == CANSM_PNC_SUPPORT)
                if ((NULL_PTR != CANSM_NETWORK_CONFIGURATION(networkIndex).TrcvRef)
                    && ((boolean)TRUE == CANSM_NETWORK_CONFIGURATION(networkIndex).TrcvRef->CanSMCanTrcvPnEnabled))
                {
...
#ifdef GEELY20_SPECIFICATION_USED
                    if (CanSM_NetWorkRunTime[netLoop].busOffCounter >= CANSM_NETWORK_CONFIGURATION(netLoop).CanSMBorCounterL1ToL2)
                    {
                    	Nm_StateChangeNotification(netLoop, 0, NHS_STATE_BUSOFF_REC_LEVEL2);
                    }
                    else
                    {
                    	Nm_StateChangeNotification(netLoop, 0, NHS_STATE_BUSOFF_REC_LEVEL1);
                    }
#endif
                    break;
                }
...
#endif
    {
        for (networkIndex = 0u; networkIndex < CANSM_NETWORK_NUM; ++networkIndex)
        {
            uint8 controllerId;
            CANSM_FOREACH_NETWORK_CONTROLLER_START(networkIndex, controllerId)
            {
        ...
        if ((uint8)E_OK == ret)
        {
            BswM_CanSM_CurrentIcomConfiguration(CANSM_NETWORK_HANDLE(networkIndex), ConfigurationId, Error);
        }
#if (STD_ON == CANSM_DEV_ERROR_DETECT)
        else
        {
...
static FUNC(void, CANSM_CODE) CanSM_FullCom_S_Bus_off_Check(uint8 netID, CanSM_NetWorkRunTimeType* networkPtr)
{
    ...
    else
    {
        ...
        if (gBusOffPassive)
        {
            ...
            Interface_ClearBusOffFlag();
#ifdef GEELY20_SPECIFICATION_USED
		uint8 CanNmState, CanNmMode;
		CanNm_GetState(netID, &CanNmState, &CanNmMode);
		Nm_StateChangeNotification(netID, 0, CanNmState);
#endif
        }
    }
}
...
#if CANSM_BUSOFF_HANDLER_MHL_ENABLE
    else if(MhlBusOffFLag == 1)
    {      
        /*恢复发送*/
        uint8 txIpduId;
        for (txIpduId = 0u; txIpduId < COM_TXIPDU_NUMBER; txIpduId++)
        {
            Com_TxIPduRunTimeState[txIpduId].ActiveEnable = TRUE;
        }
...
static FUNC(void, CANSM_CODE) CanSM_FullCom_S_Tx_Off(uint8 netID, CanSM_NetWorkRunTimeType* networkPtr)
{
    ...
    {
        busOffContinueTime = CANSM_NETWORK_CONFIGURATION(netID).CanSMBorTimeL2
#if (STD_ON == CANSM_GET_BUSOFF_DELAY_FUNCTION_USED)
                             + (uint32)networkPtr->busOffDelayTime
#endif
            ;
        Interface_SetBusOffFlag();
    }
    else
    {
        busOffContinueTime = CANSM_NETWORK_CONFIGURATION(netID).CanSMBorTimeL1
#if (STD_ON == CANSM_GET_BUSOFF_DELAY_FUNCTION_USED)
                             + (uint32)networkPtr->busOffDelayTime
#endif
            ;
...
#if CANSM_BUSOFF_HANDLER_MHL_ENABLE
        Com_TxIPduRunTimeState[(COM_TXPDU_COM_HcmlBodyExpoSignalIPDU04-COM_RXIPDU_NUMBER)].ActiveEnable = TRUE;
        Com_TxIPduRunTimeState[(COM_TXPDU_COM_HcmlBodyExpoSignalIPDU04-COM_RXIPDU_NUMBER)].Transmiting = FALSE;
        Com_TriggerIPDUSend(COM_TXPDU_COM_HcmlBodyExpoSignalIPDU04);
        Com_TxIPduRunTimeState[(COM_TXPDU_COM_HcmlBodyExpoSignalIPDU04-COM_RXIPDU_NUMBER)].ActiveEnable = FALSE;
#else
        Com_TriggerIPDUSend(COM_TXPDU_COM_HcmlBodyExpoSignalIPDU04);
#endif     
    }
...
static void CanSM_EBusOffAction(uint8 netID)
{
    ...
    if (networkPtr->curBsmState == CANSM_BSM_S_FULLCOM)
    {
        ...
        if(MhlBusOffFLag == 0)
        {
            ...
            for (txIpduId = 0u; txIpduId < COM_TXIPDU_NUMBER; txIpduId++)
            {
                Com_TxIPduRunTimeState[txIpduId].ActiveEnable = FALSE;
            }
            MhlBusOffFLag = 1;
        }
#else
...
CANSM_LOCAL void CanSM_ModeRequestTimeout(uint8 netID)
{
    ...
    if (networkPtr->repeatCounter >= CanSM_ConfigPtr->CanSMModeRequestRepetitionMax)
    {
        ...
        const CanSM_DemEventParameterRefType* demEventParameterRef =
            CANSM_NETWORK_CONFIGURATION(netID).CanSMDemEventParameterRefs;
...
#endif /* CANSM_DEM_SUPPORT == STD_ON */
        CANSM_DET_REPORTRUNTIMEERROR(SERVICE_ID_CANSM_MAINFUNCTION, CANSM_E_MODE_REQUEST_TIMEOUT);
    }
    else
    {
