...
static FUNC(void, LINIF_CODE) LinTp_HandleCopyTxDataFailure (
    P2VAR(LinTp_MasterRuntimeType, AUTOMATIC, LINIF_APPL_DATA) tpChPtr
);

static FUNC(void, LINIF_CODE) LinTp_CopyTxDataFromPduR (
    P2VAR(LinTp_MasterRuntimeType, AUTOMATIC, LINIF_APPL_DATA) tpChPtr
);

static FUNC(void, LINIF_CODE) LinTp_TxEventRequest (
    P2VAR(LinTp_MasterRuntimeType, AUTOMATIC, LINIF_APPL_DATA) tpChPtr
);

static FUNC(void, LINIF_CODE) LinTp_TxEventHandler (
    P2VAR(LinTp_MasterRuntimeType, AUTOMATIC, LINIF_APPL_DATA) tpChPtr
);

static FUNC(void, LINIF_CODE) LinTp_TxEventConfirmation (
    P2VAR(LinTp_MasterRuntimeType, AUTOMATIC, LINIF_APPL_DATA) tpChPtr
);
...
LinTp_GetChannelConfig(uint8 ch);

static FUNC(void, LINIF_CODE) LinTp_RxBufferRequestStart (
    P2VAR(LinTp_MasterRuntimeType, AUTOMATIC, LINIF_APPL_DATA) tpChPtr
);

static FUNC(void, LINIF_CODE) LinTp_RxBufferRequest (
    P2VAR(LinTp_MasterRuntimeType, AUTOMATIC, LINIF_APPL_DATA) tpChPtr
);

static FUNC(void, LINIF_CODE) LinTp_RxEventRequest (
    P2VAR(LinTp_MasterRuntimeType, AUTOMATIC, LINIF_APPL_DATA) tpChPtr
);

static FUNC(void, LINIF_CODE) LinTp_RxEventHandler (
    P2VAR(LinTp_MasterRuntimeType, AUTOMATIC, LINIF_APPL_DATA) tpChPtr
);

static FUNC(void, LINIF_CODE) LinTp_RxEventIndication (
    P2VAR(LinTp_MasterRuntimeType, AUTOMATIC, LINIF_APPL_DATA) tpChPtr
);
...
static FUNC(NetworkHandleType, LINIF_CODE) LinTp_GetLinTpChannel(
...
static VAR(LinTp_MasterRuntimeType, LINIF_VAR)
LinTp_MasterRTData[LINTP_MASTER_CHANNEL_NUMBER];
 ...
 */
/******************************************************************************/
FUNC(void,LINIF_CODE) LinTp_MasterChannelInit (
    NetworkHandleType LinIfChannelId
)
{
 ...
 */
/******************************************************************************/
FUNC(Std_ReturnType,LINIF_CODE) LinTp_MasterTransmit(
...
/******************************************************************************/
/*
 * Brief               Rx error process in master node.
 * Sync/Async          Synchronous
 * Reentrancy          Reentrant
 ...
 */
/******************************************************************************/
FUNC(void,LINIF_CODE) LinTp_MasterRxErrorHandle(
    NetworkHandleType LinIfChannelId,
    Lin_StatusType st
)
{
    ...
    /* Notify upper */
    LinIf_Det_ReportError(LINIF_MAINFUNCTION_ID,
                            LINIF_E_RESPONSE);
    #endif /* #if (LINIF_DEV_ERROR_DETECT == STD_ON) */

    if ((st == LIN_RX_ERROR) || (st == LIN_TX_HEADER_ERROR))
    {
        if (LINTP_IS_EVENT(LINTP_EVENT_RX))
        {
            /* Notify upper(PDUR) */
 ...
 */
/******************************************************************************/
FUNC(void,LINIF_CODE) LinTp_MasterTxSuccessHandle(
    NetworkHandleType LinIfChannelId
)
{
    NetworkHandleType linTpChId = LinTp_GetLinTpChannel(LinIfChannelId);
    ...
    else
    {
        ...
        tpChPtr->TpTimer.Timer =
                    tpChPtr->TxNSduPtr->LinTpNcs / LINIF_GET_TIMEBASE();
    }
 ...
 */
/******************************************************************************/
FUNC(void,LINIF_CODE) LinTp_MasterTxErrorHandle(
    NetworkHandleType LinIfChannelId,
    Lin_StatusType st
)
{
    ...
    if (LINTP_IS_EVENT(LINTP_EVENT_TX) &&
        ((st == LIN_TX_ERROR) || (st == LIN_TX_HEADER_ERROR)))
    {
        /*@req <SWS_LinIf_00069> */
        /* Notify upper(PDUR) */
        PduR_LinTpTxConfirmation(
            tpChPtr->TxNSduPtr->LinTpTxNSduPduRef,
            E_NOT_OK);
        if(TRUE == tpChPtr->ChCfgPtr->LinTpScheduleChangeDiag)
        {
            ...
            BswM_LinTp_RequestMode(
                tpChPtr->TxNSduPtr->LinTpTxNSduChannelRef,
...
FUNC(void, LINTP_CODE) LinTp_HandleTimers(NetworkHandleType LinIfChannelId)
{
        ...
        if(tpChPtr->TpTimer.Timer > 0u)
        {
            ...
            if(0u == tpChPtr->TpTimer.Timer)
            {
                    ...
                    (LINTP_TIMER_NCS == tpChPtr->TpTimer.EnabledTimer))
                {
                    ...
                    PduR_LinTpTxConfirmation(tpChPtr->TxNSduPtr->LinTpTxNSduPduRef,
                                            E_NOT_OK);
                ...
                {
                    /*@req <SWS_LinIf_00666>*/
                    PduR_LinTpRxIndication(tpChPtr->RxNSduPtr->LinTpRxNSduPduRef,
                                        E_NOT_OK);
                ...
                LinIf_ClearEvent(linIfChannel,
                                LINIF_EVENT_HEADER | LINIF_EVENT_RESPONSE);
                /* Next entry */
    ...
    if (LINTP_TIMER_NONE != tpChPtr->TpP2Timer.EnabledTimer)
    {
        if(tpChPtr->TpP2Timer.Timer > 0u)
        {
            ...
            {
                /*@req <SWS_LinIf_00619>*/
                rx = LinTp_GetRxNSduByNad(LinIfChannelId,
                                          tpChPtr->MRFRequestedNad);
 ...
 */
/******************************************************************************/
FUNC(void, LINIF_CODE) LinTp_RxEventParse (
...
)
{
    ...
    if ((NULL_PTR == tpChPtr) || (NULL_PTR == rx) || (NULL_PTR == chCfgPtr))
    {
        return;
    }
    ...
    if ((LINTP_PDU_PCI_SF == pciType) &&
        (LINTP_NEGATIVE_RSID == sdu[LINTP_PDU_OFS_SF_RSID]) &&
        ...
        (LINTP_NRC_RESPONSE_PENDING == sdu[LINTP_PDU_OFS_SF_NR_NRC]))
    {
        ...
        else
        {
            ...
            tpChPtr->TpP2Timer.Timer =
                        LINIF_GET_P2MAX_TIME / LINIF_GET_TIMEBASE();
    ...
    if (FALSE == rxCancelFlag)
    {
        /*@req <SWS_LinIf_00612>,<SWS_LinIf_00613>*/
        if ((TRUE == chCfgPtr->LinTpDropNotRequestedNad) &&
            (sdu[LINTP_PDU_OFS_NAD] != tpChPtr->MRFRequestedNad))
        {
            rxCancelFlag = TRUE;
        }
        else
        {
                ...
                (tpChPtr->LinTpChannelState != LINTP_UNINIT))
            {
                ...
                if (FALSE == isIgnoreFrame)
                {
                    ...
                    {
                        /* Data copy */
                        (void)memcpy(tpChPtr->SduBuf,sdu,LINTP_FRAME_LEN_MAX);
    ...
    if (TRUE == rxCancelFlag)
    {
        ...
        /*@req <SWS_LinIf_00646>*/
        if (TRUE == chCfgPtr->LinTpScheduleChangeDiag)
        {
            /*@req <SWS_LinIf_00655>,<SWS_LinIf_00623>*/
            BswM_LinTp_RequestMode(rx->LinTpRxNSduChannelRef,
                                   LINTP_APPLICATIVE_SCHEDULE);
            tpChPtr->RecoverMode = LINTP_APPLICATIVE_SCHEDULE;
        }
 ...
 */
/******************************************************************************/
FUNC(Std_ReturnType, LINIF_CODE)LinTp_MasterGetMRFResponse (
    NetworkHandleType LinIfChannelId,
...
)
{
    ...
    if (LINTP_IS_EVENT(LINTP_EVENT_TX_REQ) && 
        !LINTP_IS_EVENT(LINTP_EVENT_STOP_MRF))
    {
        /* Set the flag of header and response */
        LinIf_SetEvent(LinIfChannelId,
                       LINIF_EVENT_HEADER | LINIF_EVENT_RESPONSE);
        ...
        if (LINTP_IS_EVENT(LINTP_EVENT_CONF_REQ))
        {
            ...
            if (LINTP_FUNCTIONAL_REQ_NAD != tpChPtr->SduBuf[LINTP_PDU_OFS_NAD])
            {
                ...
                tpChPtr->TpP2Timer.Timer =
                        LINIF_GET_P2_TIME / LINIF_GET_TIMEBASE();
            }
        }
        ...
        tpChPtr->TpTimer.Timer =
                        tpChPtr->TxNSduPtr->LinTpNas / LINIF_GET_TIMEBASE();
        ...
        (void)memcpy(sduBufPtr, tpChPtr->SduBuf, LINTP_FRAME_LEN_MAX);
 ...
 */
/******************************************************************************/
FUNC(boolean, LINIF_CODE) LinTp_IsWaitEventSet (
    NetworkHandleType LinIfChannelId
)
{
    NetworkHandleType linTpChId = LinTp_GetLinTpChannel(LinIfChannelId);
 ...
 */
/******************************************************************************/
FUNC(boolean, LINIF_CODE) LinTp_IsStopSRFSendEventSet (
    uint8 LinIfChannelId
)
{
    NetworkHandleType linTpChId = LinTp_GetLinTpChannel(LinIfChannelId);
    ...
    if(LINTP_IS_EVENT(LINTP_EVENT_TX | LINTP_EVENT_STOP_SRF))
    {
        ret = TRUE;
    }
    else
 ...
 */
/******************************************************************************/
FUNC(boolean, LINIF_CODE) LinTp_IsStopMRFOrSRFSendEventSet (
    NetworkHandleType LinIfChannelId
)
{
    NetworkHandleType linTpChId = LinTp_GetLinTpChannel(LinIfChannelId);
    ...
    if(LINTP_IS_EVENT(LINTP_EVENT_STOP_SRF | LINTP_EVENT_STOP_MRF))
    {
        ret = TRUE;
    }
    else
...
*/
/******************************************************************************/
FUNC(boolean, LINIF_CODE) LinTp_GetScheduleChangeDiag(
    NetworkHandleType LinIfChannelId
)
{
    NetworkHandleType linTpChId = LinTp_GetLinTpChannel(LinIfChannelId);
...
)
{
    ...
    {
        /*@req <SWS_LinIf_00673>*/
        BswM_LinTp_RequestMode(tpChPtr->TxNSduPtr->LinTpTxNSduChannelRef,
                           LINTP_APPLICATIVE_SCHEDULE);
        tpChPtr->RecoverMode = LINTP_APPLICATIVE_SCHEDULE;
    }    
...
)
{
    if (tpChPtr->BufReqNum > 0u)
    {
        ...
        /* Copy data from PduR */
        bufRslt = PduR_LinTpCopyTxData(tpChPtr->TxNSduPtr->LinTpTxNSduPduRef,
                                       info,
                                       NULL_PTR,
...
{   
    ...
    if (!LINTP_IS_EVENT(LINTP_EVENT_CONF | LINTP_EVENT_OK))
    {
        ...
        /* CF */
        else if (!LINTP_IS_EVENT(LINTP_EVENT_TX_REQ) &&
                 LINTP_IS_EVENT(LINTP_EVENT_CF_REQ))
        {
...
)
{
    ...
    if (!LINTP_IS_EVENT(LINTP_EVENT_TX_REQ | LINTP_EVENT_CONF | LINTP_EVENT_OK))
    {
        ...
        if ((tpChPtr->SduRemaining != 0u) &&
            (tpChPtr->SduIdx == LINTP_FRAME_LEN_MAX))
        {
                 ...
                 (tpChPtr->SduIdx != LINTP_FRAME_LEN_MAX))
        {
            ...
            (void)memset(&tpChPtr->SduBuf[tpChPtr->SduIdx],
                   LINTP_PADDING_VALUE, len);
...
)
{
    ...
    if ((tpChPtr->SduSize > LINTP_LENGTH_SF_MIN) &&
        (tpChPtr->SduSize <= LINTP_LENGTH_SF_MAX))
    {
        /* Set events */
        LINTP_SET_EVENT(
                LINTP_EVENT_RX | LINTP_EVENT_SF_REQ | LINTP_EVENT_COPY_REQ);
...
)
{
    ...
    if (tpChPtr->SduSize >= LINTP_LENGTH_FF_MIN)
    {
        /* Set events */
        LINTP_SET_EVENT(
            LINTP_EVENT_RX | LINTP_EVENT_FF_REQ | LINTP_EVENT_COPY_REQ);
...
)
{
        ...
        && ((sdu[LINTP_PDU_OFS_PCI] & LINTP_PDU_PCI_SN_MASK) == tpChPtr->SduSN))
    {
        ...
        if (tpChPtr->SduRemaining <= 
                    (LINTP_FRAME_LEN_MAX - LINTP_PDU_OFS_SF_DATA))
        {
            /* Do not need to send a header again */
            LINTP_SET_EVENT(LINTP_EVENT_STOP_SRF);
        }
...
)
{
    ...
    else
    {
        ...
        if (TRUE == tpChPtr->ChCfgPtr->LinTpScheduleChangeDiag)
        {
            BswM_LinTp_RequestMode(tpChPtr->RxNSduPtr->LinTpRxNSduChannelRef,
                                   LINTP_APPLICATIVE_SCHEDULE);
            tpChPtr->RecoverMode = LINTP_APPLICATIVE_SCHEDULE;
        }
        ...
        LinIf_ClearEvent(tpChPtr->RxNSduPtr->LinTpLinIfChannelRef,
                         LINIF_EVENT_HEADER);
        /* Next entry */
...
)
{
    ...
    /*@req <SWS_LinIf_00674> */
    /* Copy data to PduR */
    bufRslt = PduR_LinTpCopyRxData(tpChPtr->RxNSduPtr->LinTpRxNSduPduRef,
                                   info,
                                   &(tpChPtr->UpperBufRemaining));
...
)
{
    ...
    if (LINTP_IS_EVENT(LINTP_EVENT_RX_REQ))
    {
        ...
        if ((TRUE == firstRequestBufFlag) &&
            (LINTP_IS_EVENT(LINTP_EVENT_COPY_REQ)))
        {
...
)
{
        ...
        LINTP_IS_TRS_EVENT(LINTP_TRS_EVT_FUN_TX_REQ | LINTP_TRS_EVT_PHY_TX_REQ))
    {
        ...
        /* Last Tx confirmation process may change channel state to
           IDLE->LINTP_MASTER_CH_RESET()*/
        tpChPtr->LinTpChannelState = LINTP_CHANNEL_BUSY;
