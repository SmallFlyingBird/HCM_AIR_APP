...
#define LINIF_GET_SUB_FRAME(idx)        \
            (LinIf_ConfigPtr->LinIfSubstitution[idx])
#define LINIF_GET_SUB_NUM()             \
            (LinIf_ConfigPtr->LinIfNumOfSubstitution)
#define LINIF_GET_TXPDU(idx)            \
            (LinIf_ConfigPtr->LinIfTxPdu[idx])
#define LINIF_GET_TXPDU_NUM()           \
            (LinIf_ConfigPtr->LinIfNumOfTxPdu)
#define LINIF_GET_GLOBAL_FRAME(idx)     \
            (LinIf_ConfigPtr->LinIfFrame[idx])
#define LINIF_GET_SCHEDULE_NUM(ch)      \
            (LINIF_GET_CHANNEL(ch).LinIfNumOfSchedule)
...
#define LINIF_GET_TRANSMIT_REQ(idx)  \
            (LINIF_GET_GLOBAL_FRAME(idx).LinIfIsTransmitPending[0])
#define LINIF_GET_SCHEDULE(ch,idx)      \
            (LINIF_GET_CHANNEL(ch).LinIfScheduleTable[idx])
...
#define LINIF_GET_LIN_TRCV_WAKEUP_SOURCE(ch)    \
            (LINIF_GET_CHANNEL(ch).LinIfTransceiverDrvConfig->WakeUpSource)
...
#define LINIF_HIGH_PRIORITY(a,b) \
    (((a)->LinIfSchedulePriority <= (b)->LinIfSchedulePriority) ? (a) : (b))


#if (LINIF_DEV_ERROR_DETECT == STD_ON)
#define LinIf_Det_ReportError(ApiId,ErrorId)\
