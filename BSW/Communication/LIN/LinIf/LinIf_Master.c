...
static FUNC(
P2CONST(LinIf_SubstitutionFramesType, AUTOMATIC, LINIF_CONST), LINIF_CODE)
LinIf_GetTransmitPending(
    P2CONST(LinIf_FrameType, AUTOMATIC, LINIF_APPL_CONST) frame
);

static FUNC(void, LINIF_CODE) LinIf_SendSporadic(
    uint8 ch,
    P2CONST(LinIf_FrameType, AUTOMATIC, LINIF_APPL_CONST) frame
);

static FUNC(void, LINIF_CODE) LinIf_SendEventTrigger(
    uint8 ch,
    P2CONST(LinIf_FrameType, AUTOMATIC, LINIF_APPL_CONST) frame
);

static FUNC(void, LINIF_CODE) LinIf_SendUnconditional(
    uint8 ch,
    P2CONST(LinIf_FrameType, AUTOMATIC, LINIF_APPL_CONST) frame
);
...
static FUNC(void, LINIF_CODE) LinIf_SendNodeConfiguration(
    uint8 ch,
    P2CONST(LinIf_FrameType, AUTOMATIC, LINIF_APPL_CONST) frame
);

#if ((LINIF_TP_SUPPORTED == STD_ON) && (STD_ON == LINIF_MASTER_SUPPORT))
static FUNC(void, LINIF_CODE) LinIf_SendMRF(
    uint8 ch,
    P2CONST(LinIf_FrameType, AUTOMATIC, LINIF_APPL_CONST) frame
);

static FUNC(void, LINIF_CODE) LinIf_SendSRF(
    uint8 ch,
    P2CONST(LinIf_FrameType, AUTOMATIC, LINIF_APPL_CONST) frame
);
#endif
...
static FUNC(void, LINIF_CODE) LinIf_HandleEntryTail(
...
static VAR(LinIf_MasterRuntimeType, LINIF_VAR)
LinIf_MasterChRtData[LINIF_MASTER_CHANNEL_NUMBER];
...
FUNC(void,LINIF_CODE) LinIf_MasterInit( void )
{
    ...
    for (idx = 0;idx < LINIF_MASTER_CHANNEL_NUMBER; idx++)
    {
        chPtr = LINIF_CH(idx);
 ...
 */
/******************************************************************************/
FUNC(Std_ReturnType,LINIF_CODE) LinIf_MasterTransmit(
 ...
 */
/******************************************************************************/
FUNC(Std_ReturnType,LINIF_CODE) LinIf_MasterScheduleRequest(
    NetworkHandleType ch,
...
)
{
    ...
    {
        /*@req <SWS_LinIf_00567>*/
        LinIf_Det_ReportError(LINIF_SCHEDULEREQUEST_ID,
 ...
 */
/******************************************************************************/
FUNC(void, LINIF_CODE) LinIf_MasterGotoSleep(
    NetworkHandleType ch
)
{
    P2VAR(LinIf_MasterRuntimeType, AUTOMATIC, LINIF_APPL_DATA)
    ...
    if (LINIF_IS_EVENT(LINIF_EVENT_SLEEP))
    {
        ...
        if (!LINIF_IS_EVENT(LINIF_EVENT_GOTO_SLEEP))
        {
            ...
            if (LINIF_CHANNEL_SLEEP != chPtr->LinIfChannelState)
            {
                ...
                (void)LIN_DRIVER_API(linDriver).LinGoToSleep(ch);
            ...
            else
            {
                ...
                (void)LIN_DRIVER_API(linDriver).LinGoToSleepInternal(ch);

                /*@req <SWS_LinIf_00557>*/
                /* Notify upper */
                USER_GOTO_SLEEP_CONFIRMATION(ch,
                                             LINIF_GET_COMM_NETWORK(ch),
                                             TRUE);
            }
        ...
        else
        {
            ...
            Lin_StatusType st = 
                            LIN_DRIVER_API(linDriver).LinGetStatus(ch, &sdu);
            if (LIN_CH_SLEEP == st)
            {
                /*@req <SWS_LinIf_00455>*/
                chPtr->LinIfChannelState = LINIF_CHANNEL_SLEEP;
 ...
 */
/******************************************************************************/
FUNC(void, LINIF_CODE) LinIf_MasterWakeupConfirmation(
...
FUNC(void, LINIF_CODE) LinIf_PrevTransmit(uint8 ch)
{
    ...
    if (chPtr->CurSchedule != NULL_PTR)
    {
        ...
        frame = &LINIF_GET_FRAME(ch,
                    (chPtr->CurEntry->LinIfFrameRef - LINIF_GET_FRAME_OFS(ch)));
            ...
            )
        {
            ...
            P2VAR(uint8, AUTOMATIC, LINIF_APPL_DATA) sdu = NULL_PTR;
            PduInfoType pduInfo;
            uint8 linDriver = LINIF_GET_LIN_DRIVER_ID(ch);
            Lin_StatusType st = LIN_DRIVER_API(linDriver).LinGetStatus(ch,&sdu);
                ...
                if (st == LIN_RX_OK)
                {
                    ...
                    #endif /* #if (LINIF_TP_SUPPORTED == STD_ON) */
                    {
                        ...
                        if (!LINIF_IS_EVENT(LINIF_EVENT_NCSRF_SEND))
                        {
                            ...
                            rxPduPtr->LinIfRxIndicationUL(
                                            rxPduPtr->LinIfRxPduRef, &pduInfo);
                        }
                        else
                        {
                            /*@req <SWS_LinIf_00404> */
                ...
                else
                {
                    ...
                    case LINIF_EVENT_TRIGGERED:
                        if ((st == LIN_RX_ERROR) || (st == LIN_RX_BUSY))
                        {
                            ...
                            chPtr->CollisionResolvingRef =
                                    chPtr->CurEntry->LinIfCollisionResolvingRef;
            ...
            case LINIF_TX_PDU:
                ...
                {                    
                    ...
                    case LINIF_UNCONDITIONAL:
                        ...
                        txPduPtr->LinIfTxConfirmationUL(
                                        txPduPtr->LinIfTxPduRef);
                    ...
                    case LINIF_SPORADIC:
                        ...
                        fra = &LINIF_GET_FRAME(ch,
                                    (frameIdx - LINIF_GET_FRAME_OFS(ch)));
...
FUNC(void, LINIF_CODE) LinIf_UpdateSchedule(uint8 ch)
{
    ...
    if (LINIF_IS_EVENT(LINIF_EVENT_COLLISION))
    {
        ...
        {
            chPtr->ResEntryIndex = 
                    chPtr->CurEntryIndex % chPtr->CurSchedule->LinIfNumOfEntry;
        }
        else
        {
            chPtr->ResEntryIndex = 0;
        ...
        chPtr->CurSchedule = &LINIF_GET_SCHEDULE(ch,
            chPtr->CollisionResolvingRef - LINIF_GET_SCHEDULE_OFS(ch));
        chPtr->CurEntryIndex = 0;
    }
 ...
 */
/******************************************************************************/
FUNC(Std_ReturnType, LINIF_CODE) LinIf_GetCurFrameType(
    NetworkHandleType ch,
...
*/
/******************************************************************************/
FUNC(LinIf_ChannelStateType, LINIF_CODE) LinIf_GetChannelState(
    NetworkHandleType ch
)
{
...
)
{
    ...
    if (sub != NULL_PTR)
    {
        P2CONST(LinIf_FrameType, AUTOMATIC, LINIF_APPL_CONST)
        fra = &LINIF_GET_FRAME(ch,
                (sub->LinIfSubstitutionFrameRef - LINIF_GET_FRAME_OFS(ch)));
        PduInfoType pduInfo;
        uint8 sdu[8];
        P2CONST(LinIf_TxPduType, AUTOMATIC, LINIF_APPL_DATA) txPduPtr;
        Std_ReturnType ret;
        ...
        if (E_OK == ret)
        {
            ...
            /* Send frame */
            linDriver = LINIF_GET_LIN_DRIVER_ID(ch);
            (void)LIN_DRIVER_API(linDriver).LinSendFrame(ch, &chPtr->TxPdu);
        }
        else
        {
            /*@req <SWS_LinIf_00706>*/
...
static FUNC(void, LINIF_CODE) LinIf_SwitchNewSchedule(uint8 ch)
{
    ...
    else
    {
        if (LINIF_RUN_CONTINUOUS == chPtr->CurSchedule->LinIfRunMode)
        {
            ...
            frameIdx = chPtr->RdySchedule->LinIfEntry[0].LinIfFrameRef -
                                                        LINIF_GET_FRAME_OFS(ch);
            frame = &LINIF_GET_FRAME(ch, frameIdx);
            if (((LINIF_RUN_ONCE == chPtr->RdySchedule->LinIfRunMode) &&
                 ((LINIF_MRF != frame->LinIfFrameType) &&
                  (LINIF_SRF != frame->LinIfFrameType)))
            #if ((LINIF_TP_SUPPORTED == STD_ON) && \
                 (STD_ON == LINIF_MASTER_SUPPORT))
            ||  (((LINIF_MRF == frame->LinIfFrameType) ||
                  (LINIF_SRF == frame->LinIfFrameType)) &&
            ...
            )
            {
                ...
                if(LINIF_RUN_ONCE == chPtr->RdySchedule->LinIfRunMode)
                {
                    ...
                    if (LINIF_CONTINUE_AT_IT_POINT == 
                                    chPtr->CurSchedule->LinIfResumePosition)
                    {
                        chPtr->ResEntryIndex = chPtr->CurEntryIndex % 
                                            chPtr->CurSchedule->LinIfNumOfEntry;
                    }
                    else
                    {
                        chPtr->ResEntryIndex = 0;
...
static FUNC(void, LINIF_CODE) LinIf_SwitchNewEntry(uint8 ch)
{
    ...
    if (chPtr->CurSchedule != NULL_PTR)
    {
        ...
        )
        {
            ...
            chPtr->CurEntry =
                &chPtr->CurSchedule->LinIfEntry[chPtr->CurEntryIndex];
