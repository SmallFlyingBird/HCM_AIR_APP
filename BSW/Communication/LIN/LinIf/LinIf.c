...
#define LINIF_C_AR_RELEASE_MINOR_VERSION        2U
#define LINIF_C_AR_RELEASE_REVISION_VERSION     2U
...
LinIf_GetSubstitution(
    PduIdType TxPduId
);

static FUNC(Std_ReturnType, LINIF_CODE) LinIf_CheckTxPduId(
    PduIdType TxPduId
);
...
FUNC(void, LINIF_CODE) LinIf_MasterMainHandle( void );
#endif

#if (STD_ON == LINIF_SLAVE_SUPPORT)
FUNC(void, LINIF_CODE) LinIf_SlaveMainHandle( void );
#endif
        ...
        else
        {
            ...
            for (ch = 0; ch < LINIF_NUMBER_OF_CHANNELS; ch++)
            {
                if((frameIdx >= LINIF_GET_FRAME_OFS(ch)) &&
                   (frameIdx < 
                            (LINIF_GET_FRAME_NUM(ch)+LINIF_GET_FRAME_OFS(ch))))
                {
...
)
{
    ...
    {
        /*@req <SWS_LinIf_00549>*/
        LinIf_Det_ReportError(LINIF_GETTRCVWAKEUPREASON_ID,
                              LINIF_E_NONEXISTENT_CHANNEL);
        detNoErr = FALSE;
    }
...
)
{
    ...
    {
        /*@req <SWS_LinIf_00595>*/
        LinIf_Det_ReportError(LINIF_SETTRCVWAKEUPREASON_ID,
                              LINIF_E_NONEXISTENT_CHANNEL);
        detNoErr = FALSE;
    }
...
FUNC(void, LINIF_CODE) LinIf_MainFunction(void)
{
    ...
    if ((LINIF_UNINIT == LinIf_Status)
        #if (LINIF_TP_SUPPORTED == STD_ON)
        || (LINTP_UNINIT == LinTp_Status)
        #endif /* #if (LINIF_TP_SUPPORTED == STD_ON) */
        )
    {
        /* Notify upper */
        LinIf_Det_ReportError(LINIF_MAINFUNCTION_ID, LINIF_E_UNINIT);
    }
    else
    #endif /* #if (LINIF_DEV_ERROR_DETECT == STD_ON) */
    {
...
)
{
    ...
    if ((ECUM_WKSOURCE_POWER != WakeupSource) &&
        (ECUM_WKSOURCE_RESET != WakeupSource) &&
        (ECUM_WKSOURCE_INTERNAL_RESET != WakeupSource) &&
        (ECUM_WKSOURCE_INTERNAL_WDG != WakeupSource) &&
        ...
        /*check all Lin transceiver channel to find the wake-up source*/
        {
            ...
            if (0u != (WakeupSource & wakeupSource))
            {
                /*@req <SWS_LinIf_00503>*/
                    ...
                    ret = LIN_DRIVER_API(linDriver).LinCheckWakeup(ch);
                break;
            }
        }
        }
...
{    
    ...
    {
        /*@req <SWS_LinIf_00717>*/
        LinIf_Det_ReportError(LINIF_WAKEUPCONFIRMATION_ID,
    ...
    #endif
    {
    	...
    	Com_IpduGroupVector tg_ComIpduGroupVector;
        
        #if (STD_ON == LINIF_SLAVE_SUPPORT)
        Lin_Init(&Lin_Config);
        LinIf_SlaveWakeupConfirmation(WakeupSource);
        ComM_RequestComMode(ComMChannel_RLM_RS, COMM_FULL_COMMUNICATION);
...
)
{
    ...
    #endif
    {
        ...
        if((NULL_PTR != PidBufferLength) && (*PidBufferLength > 0u))
        {
            LinIf_SlaveGetPIDTable(ch, PidBufferLength);
        }
...
)
{
    ...
    {
        /*@req <SWS_LinIf_00831>*/
        LinIf_Det_ReportError(LINIF_GETCONFIGUREDNAD_ID,
                              LINIF_E_NONEXISTENT_CHANNEL);
        detNoErr = FALSE;
    }
    if(NULL_PTR == Nad)
...
)
{
    ...
    {
        /*@req <SWS_LinIf_00835>*/
        LinIf_Det_ReportError(LINIF_SETCONFIGUREDNAD_ID,
                              LINIF_E_NONEXISTENT_CHANNEL);
        detNoErr = FALSE;
    }
    if(0u == Nad)
...
)
{
    ...
    {
        /*@req <SWS_LinIf_00849>*/
        LinIf_Det_ReportError(LINIF_RXINDICATION_ID,
                              LINIF_E_NONEXISTENT_CHANNEL);
        detNoErr = FALSE;
    }
...
)
{
    ...
    {
        /*@req <SWS_LinIf_00855>*/
        LinIf_Det_ReportError(LINIF_TXCONFIRMATION_ID,
                              LINIF_E_NONEXISTENT_CHANNEL);
    }
    else
    #endif
...
)
{
    ...
    {
        /*@req <SWS_LinIf_00855>*/
        LinIf_Det_ReportError(LINIF_LINERRORINDICATION_ID,
                              LINIF_E_NONEXISTENT_CHANNEL);
    }
    else
    #endif
...
)
{
    ...
    for (idx = 0; idx < LINIF_GET_SUB_NUM(); idx++)
    {
        ...
        frameIdx = subPtr->LinIfSubstitutionFrameRef;
        txPduPtr = 
                LINIF_GET_GLOBAL_FRAME(frameIdx).LinIfPduDirection->LinIfTxPdu;
