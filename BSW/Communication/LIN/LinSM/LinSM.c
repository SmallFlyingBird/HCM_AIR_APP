...
#define LINSM_C_AR_RELEASE_MINOR_VERSION    2
#define LINSM_C_AR_RELEASE_REVISION_VERSION 2

#define LINSM_C_SW_MAJOR_VERSION            2
#define LINSM_C_SW_MINOR_VERSION            0
#define LINSM_C_SW_PATCH_VERSION            1
...
/*@req <SWS_LinSM_00208> */
  ...
  #error "LinSM.c : Mismatch in Specification Minor Version"
#endif
#if (LINSM_C_SW_PATCH_VERSION != LINSM_H_SW_PATCH_VERSION)
  #error "LinSM.c : Mismatch in Specification Patch Version"
#endif
...
*******************************************************************************/
#define LINSM_NETWORK_HANDLE(ch)            \
            LinSM_CfgPtr->channelPtr[ch].comMNetworkHandleRef
#define LINSM_CHANNEL_SCHEDULE_NUM(ch)      \
            LinSM_CfgPtr->channelPtr[ch].scheduleNum
#define LINSM_CHANNEL_SCHEDULE_INDEX(ch,i)  \
            LinSM_CfgPtr->channelPtr[ch].schedulePtr[i].scheduleIndexRef
#define LINSM_CONFIRMATION_TIMEOUT(ch)      \
            LinSM_CfgPtr->channelPtr[ch].confirmationTimeout
#define LINSM_MODE_REQUEST_REPETITION_MAX() \
            LinSM_CfgPtr->modeRequestRepetitionMax
#define LINSM_IS_TRANSCEIVER_PASSIVER_MODE_SUPPORT(ch) \
            LinSM_CfgPtr->channelPtr[ch].isTransceiverPassiveModeSupport
#define LINSM_TRANSCEIVER_PASSIVER_MODE(ch) \
            LinSM_CfgPtr->channelPtr[ch].transceiverPassiveMode
#define LINSM_NODE_TYPE(ch) \
            LinSM_CfgPtr->channelPtr[ch].nodeType
#define LINSM_SILENCEAFTERWAKEUP_TIMEOUT(ch) \
            LinSM_CfgPtr->channelPtr[ch].silenceAfterWakeupTimeout
...
static FUNC(uint8, LINSM_CODE)
LinSM_GetChannelByNetworkHandle
(
    NetworkHandleType network
);

static FUNC(boolean, LINSM_CODE)
LinSM_CheckSchedule(
    uint8 ch,
    LinIf_SchHandleType schedule
);

static FUNC(void, LINSM_CODE)
LinSM_HandleGotoSleepPeriod(
    uint8 chl
);

static FUNC(void, LINSM_CODE)
LinSM_HandleWakeupPeriod(
    uint8 chl
);

static FUNC(void, LINSM_CODE)
LinSM_HandleChangeSchedulePeriod(
    uint8 chl
);
...
#include "LinSM_MemMap.h"
static VAR(LinSM_ModuleStatusType,LINSM_VAR_POWER_ON_INIT)
LinSM_ModuleStatus = LINSM_UNINIT;
#define LINSM_STOP_SEC_VAR_INIT_UNSPECIFIED
#include "LinSM_MemMap.h"

#define LINSM_START_SEC_VAR_CLEARED_UNSPECIFIED
#include "LinSM_MemMap.h"
static VAR(LinSM_RunTimeType, LINSM_VAR) LinSM_RTArray[LINSM_CHANNEL_NUM];
...
#include "LinSM_MemMap.h"
static P2CONST(LinSM_ConfigType, LINSM_VAR, LINSM_CONST)
LinSM_CfgPtr = NULL_PTR;
...
LinSM_Init
(
    P2CONST(LinSM_ConfigType, AUTOMATIC, LINSM_CONST_PBCFG) ConfigPtr
)
{
    uint8 loop = 0u;
    ...
    #if (STD_ON == LINSM_DEV_ERROR_DETECT)
    #if ((LINSM_VARIANT_LT == LINSM_VARIANT_CFG) || \
         (LINSM_VARIANT_PB == LINSM_VARIANT_CFG))
...
)
{
    ...
    {
        LINSM_DET_REPORT_ERROR(LINSM_SERVICE_ID_SCHEDULE_REQUEST,
    ...
    if ((E_OK == ret) && (E_NOT_OK == LinSM_CheckSchedule(chl,schedule)))
    {
        LINSM_DET_REPORT_ERROR(LINSM_SERVICE_ID_SCHEDULE_REQUEST,
                               LINSM_E_PARAMETER);
        ret = E_NOT_OK;
    ...
    if((E_OK == ret) && (LINSM_EXE_NOTHING != LinSM_RTArray[chl].runningReq))
    {
        ret = E_NOT_OK;
    }
    ...
    if ((E_OK == ret) &&
        (LINSM_FULL_COM != LinSM_RTArray[chl].channelComMode))
    {
        ret = E_NOT_OK;
    }
...
)
{
    ...
    if (LINSM_CHANNEL_NUM == chl)
    {
        LINSM_DET_REPORT_ERROR(LINSM_SERVICE_ID_GET_CURRENTCOMMODE,
                               LINSM_E_NONEXISTENT_NETWORK);
        ret = E_NOT_OK;
    }
...
)
{
    ...
    {
        LINSM_DET_REPORT_ERROR(LINSM_SERVICE_ID_REQUEST_COMMODE,
                               LINSM_E_NONEXISTENT_NETWORK);
        ret = E_NOT_OK;
    }
    ...
    if ((E_OK == ret) &&
        ...
        ((LINSM_MASTER == LINSM_NODE_TYPE(chl)) ||
         !((LINSM_SILENCEWAKEUP_TIMER == LinSM_RTArray[chl].timerType) &&
          (LinSM_RTArray[chl].timerCnt > 0u))))
    {

        ret = LinIf_Wakeup(network);

        /*@req <SWS_LinSM_00176>,<SWS_LinSM_00103> */
        if ((E_OK == ret) && (0u != LINSM_CONFIRMATION_TIMEOUT(chl)))
        {
            ...
            LinSM_RTArray[chl].wakeupRetryCnt =
                                        LINSM_MODE_REQUEST_REPETITION_MAX();
            SchM_Exit_LinSM(LINSM_INSTANCE_ID, LINSM_EXCLUSIVE_AREA_CHANNEL);
        }
    }
            ...
            if (E_OK == ret)
            {
                ...
                if (0u != LINSM_CONFIRMATION_TIMEOUT(chl))
                {
                    ...
                    LinSM_RTArray[chl].timerCnt = 
                                            LINSM_CONFIRMATION_TIMEOUT(chl);
    ...
    LinSM_RTArray[chl].reqComMode = mode;
    LinSM_RTArray[chl].reqComModeFlag = TRUE;
...
/******************************************************************************/
FUNC(void, LINSM_CODE)
LinSM_MainFunction(void)
{
    uint8 chl;
    
    for (chl = 0u; chl < LINSM_CHANNEL_NUM; chl++)
    {
...
)
{
    ...
    if (LINSM_CHANNEL_NUM == chl)
    {
        LINSM_DET_REPORT_ERROR(LINSM_SERVICE_ID_SCHEDULEREQUEST_CONFIRMATION,
                               LINSM_E_NONEXISTENT_NETWORK);
        ret = E_NOT_OK;
    }
...
)
{
    ...
    if (LINSM_CHANNEL_NUM == chl)
    {
        LINSM_DET_REPORT_ERROR(LINSM_SERVICE_ID_GOTOSLEEP_INDICATION,
                               LINSM_E_NONEXISTENT_NETWORK);
        ret = E_NOT_OK;
    }
...
)
{
    ...
    if (LINSM_CHANNEL_NUM == chl)
    {
        LINSM_DET_REPORT_ERROR(LINSM_SERVICE_ID_GOTOSLEEP_CONFIRMATION,
                               LINSM_E_NONEXISTENT_NETWORK);
        ret = E_NOT_OK;
    }
    ...
    if (E_OK == ret)
    {
        if (LINSM_EXE_GOTO_SLEEP == LinSM_RTArray[chl].runningReq)
        {
            ...
            {
                if (TRUE == LINSM_TRANSCEIVER_PASSIVER_MODE(chl)) 
                {
...
)
{
    ...
    if (LINSM_CHANNEL_NUM == chl)
    {
        LINSM_DET_REPORT_ERROR(LINSM_SERVICE_ID_WAKEUP_CONFIRMATION,
                               LINSM_E_NONEXISTENT_NETWORK);
        ret = E_NOT_OK;
    }
...
#if (CANNM_CUSTOMIZE == STD_ON)
                //EcuM_SetWakeupValidation();
#endif
...
)
{
    ...
    if( E_NOT_OK == LinSM_RTArray[chl].req2LinIfResult)
    {
        /*@req <SWS_LinSM_00177> */
        ComMode = COMM_FULL_COMMUNICATION;
        ComM_BusSM_ModeIndication(LINSM_NETWORK_HANDLE(chl), &ComMode);
        BswM_LinSM_CurrentState(LINSM_NETWORK_HANDLE(chl),
                                LINSM_FULL_COM);
        /* clear running flag */
        LinSM_RTArray[chl].runningReq = LINSM_EXE_NOTHING;
        LinSM_RTArray[chl].timerType = LINSM_NONE_TIMER;
            ...
            if(LINSM_WAKEUP_TIMER == LinSM_RTArray[chl].timerType)
            {
                ...
                else
                {
                    ...
                    if(LINSM_SLAVE == LINSM_NODE_TYPE(chl))
                    {
                        ...
                        LinSM_RTArray[chl].timerCnt =
                                LINSM_SILENCEAFTERWAKEUP_TIMEOUT(chl);
                        LinSM_RTArray[chl].timerType =
                                LINSM_SILENCEWAKEUP_TIMER;
                    }
                    else
                    {
...
)
{
    ...
    {
        /*@req <SWS_LinSM_00213> */
        BswM_LinSM_CurrentSchedule(LINSM_NETWORK_HANDLE(chl),
                                LinSM_RTArray[chl].curSchedule);
        LinSM_RTArray[chl].runningReq = LINSM_EXE_NOTHING;
        LinSM_RTArray[chl].timerType = LINSM_NONE_TIMER;
    }
    else
