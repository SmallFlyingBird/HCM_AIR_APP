...
Com_SignalUnPackHandle(const Com_SignalPackInfoType* SigPackInfoPtr, const uint8* IPduBufferPtr)
{
    uint64 signalValue = 0u;
    uint64 msbByteValue;
    uint32 bitPosition = SigPackInfoPtr->BitPosition;
    uint8* bufPtr = (uint8*)&signalValue;
    uint16 destStartIdx;
    uint16 srcStartIdx;
    uint16 sigByteStartPos = (uint16)(bitPosition >> 3u);
    uint8 tSignalByteLength = SigPackInfoPtr->SignalByteLength;
    uint8 msbByteBitSize;
    Com_SignalEndiannessType signalEndianness = SigPackInfoPtr->SignalEndianness;
    uint8 bitSize = SigPackInfoPtr->BitSize;
    Com_SignalType signalType = SigPackInfoPtr->SignalType;

    if (tSignalByteLength > 8u)
    {
    ...
    (void)Com_MemCpy(&bufPtr[destStartIdx], &IPduBufferPtr[srcStartIdx], tSignalByteLength);
    ...
    signalValue = (signalValue >> (bitPosition & 0x07u)) & (COM_UNUSED_UINT64 >> (64u - bitSize));

    /* SignalByteLength range 1..9 */
    if (SigPackInfoPtr->SignalByteLength == 9u)
    {
        /* Handle MSB byte, MSB Byte is byte 0(BigEndian) or byte 8(LittleEndian) */
        msbByteBitSize = (uint8)((bitPosition + bitSize) & 0x07u);
        if (COM_BIG_ENDIAN == signalEndianness)
        {
...
FUNC(void, COM_CODE) Com_TxSignalPackHandle(const Com_SignalPackInfoType* SigPackInfoPtr, uint64 TxValue)
{
#if (0u < COM_TXIPDUBUFF_SIZE)
    uint64 msbByteValue = TxValue;
    uint8* iPduBufferPtr = &Com_TxIPduRuntimeBuff[SigPackInfoPtr->IpduBufferIndex];
    uint32 bitPosition = SigPackInfoPtr->BitPosition;
    uint16 sigLsbBytePos = (uint16)(bitPosition >> 3u);
    uint16 sigMsbBytePos;
    uint16 destStartIdx;
    uint16 srcStartIdx;
    uint8 tSignalByteLength = SigPackInfoPtr->SignalByteLength;
    uint8 lsbPosInByte = (uint8)(bitPosition & 0x07u);
    uint64 signalValue = TxValue << lsbPosInByte;
    uint8* bufPtr = (uint8*)&signalValue;
    uint8 bitSize = SigPackInfoPtr->BitSize;
    uint8 msbByteBitSize;
    ...
    if (COM_BIG_ENDIAN == signalEndianness)
    {
        sigMsbBytePos = 1u + sigLsbBytePos - (uint16)tSignalByteLength;
    }
    else
    {
        sigMsbBytePos = sigLsbBytePos + (uint16)tSignalByteLength - 1u;
    }

    /* LSB Byte */
    bufPtr[0u] |= (iPduBufferPtr[sigLsbBytePos] & (0xFFu >> (8u - lsbPosInByte)));
    /* MSB Byte*/
    if ((0u == (bitSize & 0x07u)) && (0u == lsbPosInByte))
    {
        msbByteBitSize = 8u;
    }
    else
    ...
    {
        msbByteHighBitMask = (uint8)(0xFFu << msbByteBitSize);
        /* SignalByteLength range 1..9 */
        if (SigPackInfoPtr->SignalByteLength < 9u)
        {
            bufPtr[tSignalByteLength - 1u] |= (iPduBufferPtr[sigMsbBytePos] & msbByteHighBitMask);
        }
        else
        {
            msbByteValue = msbByteValue >> (SigPackInfoPtr->BitSize - msbByteBitSize);
            iPduBufferPtr[sigMsbBytePos] = (iPduBufferPtr[sigMsbBytePos] & msbByteHighBitMask) | (uint8)msbByteValue;
        }
    }


    tSignalByteLength = (tSignalByteLength > 8u) ? 8u : tSignalByteLength;
#if (CPU_BYTE_ORDER == LOW_BYTE_FIRST) /* Little endian byte order */
    srcStartIdx = 0u;
    if (COM_BIG_ENDIAN == signalEndianness)
    {
    ...
    if (COM_BIG_ENDIAN == signalEndianness)
    {
        destStartIdx = (uint16)1u + sigLsbBytePos - (uint16)tSignalByteLength;
    }
    else
    {
        destStartIdx = sigLsbBytePos;
    }

    (void)Com_MemCpy(&iPduBufferPtr[destStartIdx], &bufPtr[srcStartIdx], tSignalByteLength);
