...
Com_ResetRxPduTimeOut(PduIdType IpduId)
{
    ...
    for (cnt = 0u; cnt < ipduSignalRefNumber; cnt++)
    {
        ...
        Com_RxSignalTimeOutCnt[rxSignalId] = rxSignalPtr->ComFirstTimeout;
    }
#endif
#if (COM_RXSIGNALGROUP_NUMBER > 0u)
    ...
    for (cnt = 0u; cnt < ipduSignalGroupNumber; cnt++)
    {
        ...
        Com_RxSignalGroupTimeOutCnt[rxSignalGroupId] = rxSignalGroupPtr->ComFirstTimeout;
    }
#endif
    return;
}
...
Com_DisableRxPduTimeOut(PduIdType IpduId)
{
    ...
    {
        rxSignalId = rxIpduPtr->ComIPduSignalsRef[cnt];
        Com_RxSignalTimeOutCnt[rxSignalId] = 0u;
    }
#endif
#if (COM_RXSIGNALGROUP_NUMBER > 0u)
    ipduSignalGroupNumber = rxIpduPtr->ComIPduSignalGroupsRefNumber;
    ...
    {
        rxSignalGroupId = rxIpduPtr->ComIPduSignalGroupsRef[cnt];
        Com_RxSignalGroupTimeOutCnt[rxSignalGroupId] = 0u;
    }
#endif
    return;
}
...
/* PRQA S 1532 -- */ /* MISRA Rule 8.7 */
{
    ...
    Com_SignalIdType ipduSignalTimeoutRefNumber;
    Com_SignalIdType rxSignalId;
    const Com_RxSignalType* rxSignalPtr;
#endif
...
static FUNC(uint64, COM_CODE) Com_RxGroupSignalUnPack(Com_SignalIdType SignalId, uint16 IpduBufferId)
{
    ...
    sigPackInfo.BitPosition = rxGroupSignalPtr->ComBitPosition;
    sigPackInfo.SignalByteLength = rxGroupSignalPtr->ComSignalByteLength;
    sigPackInfo.SignalEndianness = rxGroupSignalPtr->ComSignalEndianness;
    sigPackInfo.BitSize = rxGroupSignalPtr->ComBitSize;
    sigPackInfo.SignalType = rxGroupSignalPtr->ComSignalType;
    signalValue = Com_SignalUnPackHandle(&sigPackInfo, &Com_RxIPduRuntimeBuff[IpduBufferId]);
...
static FUNC(uint64, COM_CODE) Com_RxSignalUnPack(Com_SignalIdType SignalId)
{
    ...
    sigPackInfo.BitPosition = rxSignalPtr->ComBitPosition;
    sigPackInfo.SignalByteLength = rxSignalPtr->ComSignalByteLength;
    sigPackInfo.SignalEndianness = rxSignalPtr->ComSignalEndianness;
    sigPackInfo.BitSize = rxSignalPtr->ComBitSize;
    sigPackInfo.SignalType = rxSignalPtr->ComSignalType;
    signalValue = Com_SignalUnPackHandle(&sigPackInfo, &Com_RxIPduRuntimeBuff[ipduBufferIndex]);
...
static FUNC(uint64, COM_CODE) Com_SourceSignalNotArrayUnPack(Com_SignalIdType SourceSignalId)
{
    ...
    sigPackInfo.BitPosition = gwSourceSignalPtr->ComBitPosition;
    sigPackInfo.SignalByteLength = gwSourceSignalPtr->ComSignalByteLength;
    sigPackInfo.SignalEndianness = gwSourceSignalPtr->ComSignalEndianness;
    sigPackInfo.BitSize = gwSourceSignalPtr->ComBitSize;
    sigPackInfo.SignalType = gwSourceSignalPtr->ComSignalType;
    signalValue = Com_SignalUnPackHandle(&sigPackInfo, &Com_RxIPduRuntimeBuff[ipduBufferIndex]);
