...
/**
  ...
  RuleSorce:puhua-rule.rcf 2.3.1
    ...
    \li PRQA S 1496 MISRA Rule 21.15 .<br>
...
#define COM_C_SW_PATCH_VERSION 8u
/*******************************************************************************
**                       Version  Check                                       **
*******************************************************************************/
...
#include "Com_MemMap.h"
VAR(uint16, COM_VAR)
Com_RxSignalTimeOutCnt[COM_RXSIGNAL_NUMBER];
...
#include "Com_MemMap.h"
VAR(uint16, COM_VAR)
Com_RxSignalGroupTimeOutCnt[COM_RXSIGNALGROUP_NUMBER];
#define COM_STOP_SEC_VAR_NO_INIT_16
#include "Com_MemMap.h"
#endif
...
#if (COM_RXSIGNAL_NUMBER > 0u)
        for (cirCnt = 0u; cirCnt < COM_RXSIGNAL_NUMBER; cirCnt++)
        {
            Com_RxSignalTimeOutCnt[cirCnt] = 0u;
        }
#endif /*COM_RXSIGNAL_NUMBER > 0u*/
#if (COM_RXSIGNALGROUP_NUMBER > 0u)
        for (cirCnt = 0u; cirCnt < COM_RXSIGNALGROUP_NUMBER; cirCnt++)
        {
            Com_RxSignalGroupTimeOutCnt[cirCnt] = 0u;
        }
...
#endif
    {
        vectorByteLength = ((COM_IPDUGROUP_NUMBER - 1u) >> 3u) + 1u;
...
#endif /* #if (STD_ON == COM_DEV_ERROR_DETECT) */
                    }
...
static FUNC(void, COM_CODE) Com_RxSignalTimeOutHandle(PduIdType rxIpduId)
{
    Com_SignalIdType ipduSignalTimeoutRefNumber, cnt;
    ...
    for (cnt = 0u; cnt < ipduSignalTimeoutRefNumber; cnt++)
    {
        rxSignalId = rxIpduPtr->ComIPduSignalsTimeoutRef[cnt];
        rxSignalPtr = &Com_ConfigStd->ComRxSignal[rxSignalId];
        if (Com_RxSignalTimeOutCnt[rxSignalId] > 0u)
        {
            (Com_RxSignalTimeOutCnt[rxSignalId]) -= 1u;
            if (0u == Com_RxSignalTimeOutCnt[rxSignalId])
            {
/*let pass any value for the filter MASKED_NEW_DIFFERS_MASKED_OLD after an RX deadline timeout*/
                ...
                Com_RxSignalTimeOutCnt[rxSignalId] = rxSignalPtr->ComTimeout;
            }
        }
    }
    return;
...
static FUNC(void, COM_CODE) Com_RxSignalGroupTimeOutHandle(PduIdType rxIpduId)
{
    ...
    uint16 initValueLength;
    uint16 initValueId;
    Com_SignalGroupIdType ipduSignalGroupTimeoutNumber;
    ...
    for (cnt = 0u; cnt < ipduSignalGroupTimeoutNumber; cnt++)
    {
        rxSignalGroupId = rxIpduPtr->ComIPduSignalGroupsTimeoutRef[cnt];
        rxSignalGroupPtr = &Com_ConfigStd->ComRxSignalGroup[rxSignalGroupId];
        if (Com_RxSignalGroupTimeOutCnt[rxSignalGroupId] > 0u)
        {
            (Com_RxSignalGroupTimeOutCnt[rxSignalGroupId]) -= 1u;
            if (0u == Com_RxSignalGroupTimeOutCnt[rxSignalGroupId])
            {
...
/*replace the rx pdu buffer for signal group array*/
        ...
        */
                ...
                Com_RxSignalGroupTimeOutCnt[rxSignalGroupId] = rxSignalGroupPtr->ComTimeout;
            }
        }
    }
}
    ...
    Com_ReceiveSignalHandle(Com_SignalIdType SignalId, P2VAR(void, AUTOMATIC, COM_APPL_CONST) SignalDataPtr)
{
    ...
    if (FALSE == Com_RxIPduRunTimeState[ipduRef].Receiving)
    {
        ...
        case COM_UINT8:
        case COM_SINT8:
            (void)memcpy(SignalDataPtr, &Com_Signal8BitRuntimeBuff[signalInitValueId], sizeof(uint8));
            break;
        case COM_UINT8_N:
            signalLength = rxSignalPtr->ComSignalLength;
            /* Copy Uint8N signal value one byte by one */
            (void)memcpy(SignalDataPtr, &Com_Signal8BitRuntimeBuff[signalInitValueId], signalLength);
            break;
#endif
#if (0u < COM_SIGNAL_16BITBUFF_SIZE)
        case COM_UINT16:
        case COM_SINT16:
            (void)memcpy(SignalDataPtr, &Com_Signal16BitRuntimeBuff[signalInitValueId], sizeof(uint16));
            break;
#endif
#if (0u < COM_SIGNAL_32BITBUFF_SIZE)
        case COM_SINT32:
        case COM_UINT32:
        case COM_FLOAT32:
            (void)memcpy(SignalDataPtr, &Com_Signal32BitRuntimeBuff[signalInitValueId], sizeof(uint32));
            break;
#endif
#if (0u < COM_SIGNAL_64BITBUFF_SIZE)
        case COM_SINT64:
        case COM_UINT64:
        case COM_FLOAT64:
            (void)memcpy(SignalDataPtr, &Com_Signal64BitRuntimeBuff[signalInitValueId], sizeof(uint64));
            break;
#endif
        default:
            /*do nothing*/
...
/* PRQA S 1496 ++ */ /* MISRA Rule 21.15 */
        ...
        case COM_UINT8:
        case COM_SINT8:
            (void)memcpy(SignalDataPtr, &Com_RxGroupSignal8BitShadowBuff[signalInitValueId], sizeof(uint8));
            break;
        case COM_UINT8_N:
            signalLength = rxGroupSignalPtr->ComSignalLength;
            /* Copy Uint8N signal value one byte by one */
            (void)memcpy(SignalDataPtr, &Com_RxGroupSignal8BitShadowBuff[signalInitValueId], signalLength);
            break;
#endif
/* PRQA S 1496 -- */ /* MISRA Rule 21.15 */
#if (0u < COM_RXGROUPSIGNAL_16BITBUFF_SIZE)
        case COM_UINT16:
        case COM_SINT16:
            (void)memcpy(SignalDataPtr, &Com_RxGroupSignal16BitShadowBuff[signalInitValueId], sizeof(uint16));
            break;
#endif
#if (0u < COM_RXGROUPSIGNAL_32BITBUFF_SIZE)
        case COM_SINT32:
        case COM_UINT32:
        case COM_FLOAT32:
            (void)memcpy(SignalDataPtr, &Com_RxGroupSignal32BitShadowBuff[signalInitValueId], sizeof(uint32));
            break;
#endif
#if (0u < COM_RXGROUPSIGNAL_64BITBUFF_SIZE)
        case COM_SINT64:
        case COM_UINT64:
        case COM_FLOAT64:
            (void)memcpy(SignalDataPtr, &Com_RxGroupSignal64BitShadowBuff[signalInitValueId], sizeof(uint64));
            break;
#endif
        default:
            /*do nothing*/
    ...
    if (COM_UINT8_DYN == Com_ConfigStd->ComRxSignal[SignalId].ComSignalType)
    {
        ...
        if (FALSE == Com_RxIPduRunTimeState[ipduRef].Receiving)
        {
            ...
            else
            {
                ...
                (void)memcpy((uint8*)SignalDataPtr, &Com_Signal8BitRuntimeBuff[signalInitValueId], signalLength);
                /* PRQA S 1496 */ /* MISRA Rule 21.15 */
                *Length = signalLength;
                /* Check IPdu which this group signal belonged is Active or not*/
                if (TRUE == Com_RxIPduRunTimeState[ipduRef].ActiveEnable)
