...
#define COM_MEMCPY_FUN_SELECT  COM_USE_STD_LIB_MEMCPY
...
FUNC(uint64, COM_CODE) Com_SignalUnPackHandle(const Com_SignalPackInfoType* SigPackInfoPtr, const uint8* IPduBufferPtr);
...
extern FUNC(void, COM_CODE) Com_TxSignalPackHandle(const Com_SignalPackInfoType* SigPackInfoPtr, uint64 TxValue);
#endif
 ...
 */
/******************************************************************************/
COM_LOCAL_INLINE void Com_MemCpy(void* dest, const void* src, uint16 n)
{
#if (COM_MEMCPY_FUN_SELECT == COM_USE_STD_LIB_MEMCPY)
    (void)memcpy((uint8*)dest, (const uint8*)src, n);
#else
