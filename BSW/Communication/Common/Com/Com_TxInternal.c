...
/**
  ...
  RuleSorce:puhua-rule.rcf 2.3.1
    ...
    \li PRQA S 1487,1496 MISRA Rule 21.15 .<br>
    Reason:Destination and source objects may have different types.
 */
...
{
    boolean ret = FALSE;
    /* PRQA S 1487,1496 ++ */ /* MISRA Rule 21.15 */
    switch (SignalType)
    {
    ...
    case COM_UINT8:
    case COM_SINT8:
        if (0 != memcmp(&Com_Signal8BitRuntimeBuff[SignalBufferId], SignalNewDataPtr, sizeof(uint8)))
        {
            (void)memcpy(&Com_Signal8BitRuntimeBuff[SignalBufferId], SignalNewDataPtr, sizeof(uint8));
            ret = TRUE;
        }
        break;
    case COM_UINT8_N:
        /* Copy Uint8N signal value one byte by one */
        if (0 != memcmp(&Com_Signal8BitRuntimeBuff[SignalBufferId], SignalNewDataPtr, SignalLength))
        {
            (void)memcpy(&Com_Signal8BitRuntimeBuff[SignalBufferId], SignalNewDataPtr, SignalLength);
            ret = TRUE;
        }
        break;
#endif
    ...
    case COM_UINT16:
    case COM_SINT16:
        if (0 != memcmp(&Com_Signal16BitRuntimeBuff[SignalBufferId], SignalNewDataPtr, sizeof(uint16)))
        {
            (void)memcpy(&Com_Signal16BitRuntimeBuff[SignalBufferId], SignalNewDataPtr, sizeof(uint16));
            ret = TRUE;
        }
        break;
#endif
    ...
    case COM_UINT32:
    case COM_FLOAT32:
        if (0 != memcmp(&Com_Signal32BitRuntimeBuff[SignalBufferId], SignalNewDataPtr, sizeof(uint32)))
        {
            (void)memcpy(&Com_Signal32BitRuntimeBuff[SignalBufferId], SignalNewDataPtr, sizeof(uint32));
            ret = TRUE;
        }
        break;
#endif
    ...
    case COM_UINT64:
    case COM_FLOAT64:
        if (0 != memcmp(&Com_Signal64BitRuntimeBuff[SignalBufferId], SignalNewDataPtr, sizeof(uint64)))
        {
            (void)memcpy(&Com_Signal64BitRuntimeBuff[SignalBufferId], SignalNewDataPtr, sizeof(uint64));
            ret = TRUE;
        }
        break;
#endif
...
Com_TxSignalPack(Com_SignalIdType TxSignalId) /* PRQA S 1532 */ /* MISRA Rule 8.7 */
{
    ...
    uint64 txData;
    Com_SignalPackInfoType parameter;
#if (                                                                                          \
    ...
    if (COM_UINT8_N == signalType)
    {
        ...
        (void)Com_MemCpy(iPduBufferPtr, &Com_Signal8BitRuntimeBuff[signalBufferId], signalLength);
    ...
    else if (COM_BOOLEAN == signalType)
    {
        ...
        uint8 value = (uint8)(1u << ((uint8)bitPosition & 0x07u));
    ...
    else
    {
        ...
        parameter.SignalByteLength = txSignalPtr->ComSignalByteLength;
        ...
        parameter.BitSize = txSignalPtr->ComBitSize;
        Com_TxSignalPackHandle(&parameter, txData);
    }
}
#endif /*0u < COM_TXSIGNAL_NUMBER*/
/******************************************************************************/
...
/*dynamic signal isn't send by the API,InitialValueOnly signal group don't expect to be called by any API*/
    ...
    {
        uint16 groupSignalBufferId = txGroupSignalPtr->ComSignalInitValueId;
#if (0u < COM_TMCTXGROUPSIGNAL_NUMBER) && (STD_ON == COM_TMS_ENABLE)
...
#if (STD_OFF == COM_TX_SIG_GROUP_PROP_TRIG_ON_CH_NO_REPETITION_ENABLE) \
    && (STD_OFF == COM_TX_SIG_GROUP_PROP_TRIGGERED_ON_CHANGE_ENABLE)
    COM_NOUSED(valueChanged);
#endif /* (STD_OFF == COM_TX_SIG_GROUP_PROP_TRIG_ON_CH_NO_REPETITION_ENABLE) \
    && (STD_OFF == COM_TX_SIG_GROUP_PROP_TRIGGERED_ON_CHANGE_ENABLE) */
    return ret;
}
#endif
...
#if (0u < COM_SIGNAL_8BITBUFF_SIZE)
    /* PRQA S 1487,1496 ++ */ /* MISRA Rule 21.15 */
    if (0 != memcmp(&Com_Signal8BitRuntimeBuff[SignalBufferId], SignalNewDataPtr, Length))
    {
        (void)memcpy(&Com_Signal8BitRuntimeBuff[SignalBufferId], SignalNewDataPtr, Length);
        ret = TRUE;
    }
    /* PRQA S 1487,1496 -- */ /* MISRA Rule 21.15 */
#endif
...
#if (                                                                     \
    ((0u < COM_TMCTXGROUPSIGNAL_NUMBER) && (0u < COM_TMCTXSIGNAL_NUMBER)) \
    || (STD_ON == COM_TX_GRP_SIGNAL_TYPE_UINT8_DYN_ENABLE))
    const Com_TxGroupSignalType* txGroupSignalPtr;
#endif /* (((0u < COM_TMCTXGROUPSIGNAL_NUMBER) && (0u < COM_TMCTXSIGNAL_NUMBER)) || (STD_ON == \
          COM_TX_GRP_SIGNAL_TYPE_UINT8_DYN_ENABLE)) */
...
static FUNC(void, COM_CODE) Com_TxGroupSignalPack(Com_SignalIdType GroupSignalId)
{
    ...
    else
    {
        ...
        parameter.SignalByteLength = txGroupSignalPtr->ComSignalByteLength;
        parameter.SignalEndianness = txGroupSignalPtr->ComSignalEndianness;
        parameter.IpduBufferIndex = ipduBufferIndex;
        parameter.BitPosition = bitPosition;
        parameter.BitSize = txGroupSignalPtr->ComBitSize;
        Com_TxSignalPackHandle(&parameter, txData);
    }
}
#endif /* COM_TXSIGNALGROUP_NUMBER > 0u */
...
static FUNC(uint64, COM_CODE) Com_UnPackTxArrayGroupSignalValue(Com_SignalIdType SignalId, uint16 IpduBufferId)
{
    ...
    uint64 signalValue;
    Com_SignalPackInfoType sigPackInfo;

    sigPackInfo.BitPosition = txGroupSignalPtr->ComBitPosition;
    sigPackInfo.SignalByteLength = txGroupSignalPtr->ComSignalByteLength;
    sigPackInfo.SignalEndianness = txGroupSignalPtr->ComSignalEndianness;
    sigPackInfo.BitSize = txGroupSignalPtr->ComBitSize;
    sigPackInfo.SignalType = txGroupSignalPtr->ComSignalType;
    signalValue = Com_SignalUnPackHandle(&sigPackInfo, &Com_TxIPduRuntimeBuff[IpduBufferId]);
    ...
    Com_GetTxSignalValue(Com_SignalType SignalType, P2CONST(void, AUTOMATIC, COM_APPL_CONST) SignalValuePtr)
{
    ...
    case COM_UINT8:
    case COM_SINT8:
        (void)memcpy((uint8*)&signalNewValue, (const uint8*)SignalValuePtr, sizeof(uint8));
        break;
    case COM_SINT16:
    case COM_UINT16:
        (void)memcpy((uint16*)&signalNewValue, (const uint16*)SignalValuePtr, sizeof(uint16));
        break;
    case COM_SINT32:
    case COM_UINT32:
        (void)memcpy((uint32*)&signalNewValue, (const uint32*)SignalValuePtr, sizeof(uint32));
        break;
    case COM_SINT64:
    case COM_UINT64:
        (void)memcpy((uint64*)&signalNewValue, (const uint64*)SignalValuePtr, sizeof(uint64));
        break;
    default:
        /*do nothing*/
        break;
