...
#define PDUR_C_SW_PATCH_VERSION 2u
/*******************************************************************************
**                       Version  Check                                       **
*******************************************************************************/
        ...
        if (!PDUR_ROUTINGPATH_CFG(srcPduId).TpRoute)
        {
            ...
            if (!destPduConfigPtr->GateWayRoute)
            {
                ...
                if ((NULL_PTR != pduR_UpIfTxConfirmationApi) && (srcPduConfigPtr->PduRSrcPduUpTxConf))
                {
                    ...
                    if (PDUR_UNUSED_UINT16 == upTxStateId)
                    {
    ...
    PduLengthType PduLength)
{
    ...
    (void)memcpy(TxBufferPtr->TxBufferData, PduInfo->SduDataPtr, PduLength);
#if (STD_ON == PDUR_META_DATA_SUPPORT)
    if (0u < MetaDataLength)
    {
        (void)memcpy(TxBufferPtr->MetaData, PduInfo->MetaDataPtr, (PduLengthType)MetaDataLength);
    }
#endif
    return;
}
