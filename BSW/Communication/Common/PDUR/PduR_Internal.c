    ...
    if (PduR_GateWayDestTpRTSate[gatewayTpRunTimeId].TpTxConfirmation)
    {
...
#endif /*PDUR_TX_BUFFER_SUM > 0u*/
        {
            ...
            if (PDUR_UNUSED_UINT8 != findTpBuffer)
            {
                ...
                {
                    (void)
                        memcpy(PduR_TpBuffer[findTpBuffer].MetaData, info->MetaDataPtr, (PduLengthType)metaDataLength);
                }
#endif
                if ((NULL_PTR != info) && (NULL_PTR != info->SduDataPtr))
                {
...
#endif
    if ((PDUR_UNUSED_UINT16 != gateWayTpRunTimeId) && (!PduR_GateWayDestTpRTSate[gateWayTpRunTimeId].TpTxConfirmation))
    {
#if (PDUR_TX_BUFFER_SUM > 0u)
        pduRDestTxBuffeId = PDUR_DESTPDU_CFG(DestPduId).PduRDestTxBufferRef;
...
/* PRQA S 3432 -- */ /* MISRA Rule 20.7 */
{
    ...
    {
        (void)memcpy(&PduR_TpBuffer[TpBufferId].TpBufferData[rxBufferOffset], Info->SduDataPtr, Info->SduLength);
