...
static FUNC(void, DCM_CODE) DspInternal_DcmConfirmForRePrg(uint8 ProtocolCtrlId)
{
    ...
    for (iloop = 0; iloop < Dcm_DspCfg.pDcm_DspSession->DcmDspSessionRow_Num; iloop++)
    {
        ...
        {
            if ((pSessionRow->DcmDspSessionForBoot == DCM_OEM_BOOT)
                || (pSessionRow->DcmDspSessionForBoot == DCM_SYS_BOOT))
            {
                ...
                || (pSessionRow->DcmDspSessionForBoot == DCM_SYS_BOOT_RESPAPP))
            {
                ...
                Dcm_Channel[Offset + 2u] = (uint8)(pSessionRow->DcmDspSessionP2ServerMax >> 8u);
                Dcm_Channel[Offset + 3u] = (uint8)(pSessionRow->DcmDspSessionP2ServerMax);
                Dcm_Channel[Offset + 4u] = (uint8)((pSessionRow->DcmDspSessionP2StarServerMax) >> 8u);
                Dcm_Channel[Offset + 5u] = (uint8)(pSessionRow->DcmDspSessionP2StarServerMax);
