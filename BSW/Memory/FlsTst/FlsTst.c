...
#define FLSTST_C_AR_MINOR_VERSION 2U
#define FLSTST_C_AR_PATCH_VERSION 2U
#define FLSTST_C_SW_MAJOR_VERSION 1U
#define FLSTST_C_SW_MINOR_VERSION 0U
#define FLSTST_C_SW_PATCH_VERSION 0U
/*============================================================================*/
...
#if(202014U != __GHS_VERSION_NUMBER)&&((202114U != __GHS_VERSION_NUMBER))
...
#define FLSTST_START_SEC_VAR_NOINIT_8BIT
#include "FlsTst_MemMap.h"
uint8 CrcCompleted;
#define FLSTST_STOP_SEC_VAR_NOINIT_8BIT
#include "FlsTst_MemMap.h"
/*============================================================================*/
...
void FlsTst_Init (const FlsTst_ConfigType* ConfigPtr)
{
    ...
    FlashInfo = REG_READ32(FLASH_MSCM_OCMDR0_ADDR32);
    /*FlashTest only support S32K146*/
    if(0xCB089000U == FlashInfo) /*0xDC089000*/
	{
		/*nothing to do*/
	}
    else
        ...
        else
        {
            ...
            for(MemBlockIndex = 0; MemBlockIndex < FLSTST_BGND_BLOCK_NUM; MemBlockIndex++)
            {
                ...
                else
                {
                    ...
                    {
                    	/* Error parameter stored at fixed address锛孭lease check signature have been stored at correct address */
                    	FLSTST_DET_REPORTERROR(FLSTST_INIT_ID, FLSTST_E_INIT_FAILED);
                    }
                    else
                    {
...
#endif
                FlsTst_BlockBgndInfo[MemBlockIndex].MemoryBlockID = ConfigPtr->FlsTst_BgndBlockPtr[MemBlockIndex].BgndBlockIndex;
                FlsTst_BlockBgndInfo[MemBlockIndex].TestAlgorithm = ConfigPtr->FlsTst_BgndBlockPtr[MemBlockIndex].TestAlgorithm;
                FlsTst_BlockBgndInfo[MemBlockIndex].MemBlcokStartAddr = ConfigPtr->FlsTst_BgndBlockPtr[MemBlockIndex].BlockBaseAddress;
                FlsTst_BlockBgndInfo[MemBlockIndex].MemBlcokLength = ConfigPtr->FlsTst_BgndBlockPtr[MemBlockIndex].BlcokLength;
                if(FLSTST_DUPLICATED_MEMORY == FlsTst_BlockBgndInfo[MemBlockIndex].TestAlgorithm)
                {
                    /*Duplicated memory start address*/
                    FlsTst_BlockBgndInfo[MemBlockIndex].SignatureStored = ConfigPtr->FlsTst_BgndBlockPtr[MemBlockIndex].SignatureAddress;
                }
                else
                {
...
#endif
                FlsTst_BlockFgndInfo[MemBlockIndex].MemoryBlockID = ConfigPtr->FlsTst_FgndBlockPtr[MemBlockIndex].BgndBlockIndex;
                FlsTst_BlockFgndInfo[MemBlockIndex].TestAlgorithm = ConfigPtr->FlsTst_FgndBlockPtr[MemBlockIndex].TestAlgorithm;
                FlsTst_BlockFgndInfo[MemBlockIndex].MemBlcokStartAddr = ConfigPtr->FlsTst_FgndBlockPtr[MemBlockIndex].BlockBaseAddress;
                FlsTst_BlockFgndInfo[MemBlockIndex].MemBlcokLength = ConfigPtr->FlsTst_FgndBlockPtr[MemBlockIndex].BlcokLength;
                if(FLSTST_DUPLICATED_MEMORY == FlsTst_BlockFgndInfo[MemBlockIndex].TestAlgorithm)
                {
                    /*Duplicated memory start address*/
                    FlsTst_BlockFgndInfo[MemBlockIndex].SignatureStored = ConfigPtr->FlsTst_FgndBlockPtr[MemBlockIndex].SignatureAddress;
                }
                else
                {
...
static void FlsTst_CopyBlockInfo(FlsTst_BlockIdType FlsTstBlockID)
{
    ...
    else
    {
        ...
        SchM_Exit_FlsTst_RAMTST_EXCLUSIVE_AREA();
#if(STD_ON == FLSTST_EXTEND_SAFETY_MODE )
        FlsTst_ParamChecksum = (uint32)FlsTst_CurRunning.TestAddr + (uint32)FlsTst_CurRunning.TestLength + (uint32)FlsTst_CurRunning.CalcResult;
#endif
    }
}
#define FLSTST_STOP_SEC_CODE
...
static void FlsTst_InterTest(void)
{
    ...
    else
    {
		...
		if((FLSTST_TESTCELLS_NUM * FLSTST_TESTCELL_LENGTH_ATOMIC) < FlsTst_CurRunning.TestLength)
		{
			...
			for(l_Counter = 0; l_Counter < Cycletimes;l_Counter++)
			{
...
static void FlsTst_CalcAtomic(void)
{
...
#if (STD_ON == FLSTST_EXTEND_SAFETY_MODE)
 	if((uint32)FlsTst_ParamChecksum != ((uint32)FlsTst_CurRunning.TestAddr + (uint32)FlsTst_CurRunning.TestLength + (uint32)FlsTst_CurRunning.CalcResult))
 	{
    ...
    case FLSTST_16BIT_CRC:
        FlsTst_CurRunning.CalcResult = (uint32)Crc_CalculateCRC16((uint8 *)FlsTst_CurRunning.TestAddr, l_CrcLength, (uint16)FlsTst_CurRunning.CalcResult,l_FirstTimeCalc);
        break;

    case FLSTST_32BIT_CRC:
        FlsTst_CurRunning.CalcResult = (uint32)Crc_CalculateCRC32((uint8 *)FlsTst_CurRunning.TestAddr, l_CrcLength, (uint32)FlsTst_CurRunning.CalcResult,l_FirstTimeCalc);
        break;

    case FLSTST_8BIT_CRC:
        FlsTst_CurRunning.CalcResult = (uint32)Crc_CalculateCRC8((uint8 *)FlsTst_CurRunning.TestAddr, l_CrcLength, (uint8)FlsTst_CurRunning.CalcResult,l_FirstTimeCalc);
        break;

    case FLSTST_CHECKSUM:
        FlsTst_CurRunning.CalcResult = (uint32)FlsTst_CheckSum((uint8 *)FlsTst_CurRunning.TestAddr, l_CrcLength, (uint32)FlsTst_CurRunning.CalcResult,l_FirstTimeCalc);
        break;
...
static Std_ReturnType FlsTst_CRC32Fgnd(FlsTst_BlockInfoType* BlockInfoPtr)
{
    ...
    else
    {
		...
		/*Specifies the start address of Foreground test*/
		testBuffptr = (uint8 *)TestAddr;
