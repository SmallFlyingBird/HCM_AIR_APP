...
const uint8 ADNode_mapping[10] = {
    A_D_Thermal,A_D_VPIN, A_D_VSNSN1, A_D_VSNSN2};
/*==================================================================================================
*                                      LOCAL VARIABLES
==================================================================================================*/
static uint8 BD18397LostConfigFlag[2] = { 0, 0};
static uint8 BD18397LostComFlag[6]={0};/* 检测18397丢通信  */
...
static BD18397_RegDataType BD18397RegData[2] = {
    {/*INIT VAL*/
     ...
     .BD18397_SYSSET_Data = 0xC0,//0x80|0x40
...
static Std_ReturnType BD18397Transmit(BD18397_TransType *TransData, BD18397_ReceiveType *ReceiveData, uint8 is10bit, uint8 isContinuous)
{
    ...
    if ((TransData->RWAddr & 0x80) == 0)
    {
        ...
        if(res==E_NOT_OK)
        {
            if(BD18397LostComFlag[TransData->ID]<=200) //the max crc err is 200
            {
                BD18397LostComFlag[TransData->ID]++;
            }
        }
        else
        {
            BD18397LostComFlag[TransData->ID]=0;
        }
    }
    else
    {
...
Std_ReturnType BD18397MainFun(uint8 id)
{
    ...
    else
    {
        ...
        BD18397_ADCOrignalval[id].AdcStruct.ADSEL=1;
        BD18397_ADCOrignalval[id].AdcStruct.TMEPDELAY++;
        if(BD18397_ADCOrignalval[id].AdcStruct.TMEPDELAY>=10)
        {
            BD18397_ADCOrignalval[id].AdcStruct.TMEPDELAY=0;
...
{
    if(BD18397LostComFlag[id] >=5)
    {
       *val = 1; 
    }
    else if(BD18397LostComFlag[id] == 0)
    {
        *val = 0;
    }
    return E_OK;
