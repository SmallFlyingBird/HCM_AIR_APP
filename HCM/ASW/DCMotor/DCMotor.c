...
/* 如果进入E2E，禁止对电机进行调整 */
    ...
    if(gs_DCMotorRunInfo.LastStartupTime < Get_pDCMotrDeactDlyTi())
    {
...
/* 电机运行时间由参数表参数决定 */
        if(gs_DCMotorRunInfo.LastStartupTime >= Get_pDCMotrDeactDlyTi()) /* 关闭高边之前需要等待上次启动完成 */
        {
    ...
    if(gs_DCMotorRunInfo.PosPwm_Last != gs_DCMotorRunInfo.PosPwm_Curr)
    {
        ...
        else if (gs_DCMotorRunInfo.PosPwm_Last >= gs_DCMotorConfigInfo.LVLSafetyPos && gs_DCMotorRunInfo.PosPwm_Curr >= gs_DCMotorConfigInfo.LVLSafetyPos) /* 调档 */
        {
            ...
            else
            {
                if(gs_DCMotorRunInfo.LastStartupTime > Get_pDCMotrDeactDlyTi())
                {
                    HsdEndCheckTime = Get_pDCMotrDeactDlyTi();
                }
                else
                {
...
Std_ReturnType DCMotor_GetSIGErrStatus(void)
{
    ...
    {
        if((gs_DCMotorRunInfo.ErrStatus.Bits.CtrLine!=0)||(gs_DCMotorRunInfo.ErrStatus.Bits.Stall!=0)||(gs_DCMotorRunInfo.ErrStatus.Bits.Signal!=0))
        {
            rtval= E_NOT_OK;/* 有故障就保持，直到恢复故障 */
        }
        else
        {
            rtval=E_OK;
        ...
        {
            Interface_SetD900Data( HSD2_SCB_OL , STATE_ERROR);
            Interface_SetD900Data( DC_Motor_Fail , STATE_ERROR);
        }
        else 
        {
            Interface_SetD900Data( HSD2_SCB_OL , STATE_OK);
            if((gs_DCMotorRunInfo.ErrStatus.Bits.Stall!=0)||(gs_DCMotorRunInfo.ErrStatus.Bits.Signal!=0))
            {
                Interface_SetD900Data( DC_Motor_Fail , STATE_ERROR);
            }
            else
            {
                Interface_SetD900Data( DC_Motor_Fail , STATE_OK);
            }
        }
    }
    return rtval;
...
static Std_ReturnType DCMotor_CtrLineDtcErrDetect(void)
{
        ...
        (gs_DCMotorRunInfo.ErrStatus.Status & 0x0F) == 0u )
    {
        ...
        if(se_OLDetEnFlag == E_EnableFlag_ENABLE)
        {
            if(gs_DCMotorRunInfo.LastStartupTime < gs_DCMotorConfigInfo.DeactDlyTi)
            {
                if(HSDManage_GetHSDOutputCurrent(gs_DCMotorConfigInfo.HSChannel) > 15) /* 统计电流大于15mA的数量 */
                {
...
/* 控制线开路和HSD开路的检测结果都是HSD输出电流为0 */
        if(s_OLErrNum >= 2u)
        {
            Interface_SetDtcHSDAndFanError((E_HSDAndDCMotorErrorType_HSD1_OpenOrShort2Vcc), 1);
        }
        else if(s_OLErrNum == 0u)
        {
            Interface_SetDtcHSDAndFanError((E_HSDAndDCMotorErrorType_HSD1_OpenOrShort2Vcc), 0);
        }
