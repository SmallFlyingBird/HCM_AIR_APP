...
static void Set_DynSignal(void)
{
    ...
    && ((E2eError.bits.ActvnOfIndcrTimeout==0))
    #endif
    )
    {
...
/* 如果CH1bypass用于远光，当远光打开的时候，近光也打开 */
    if(0 != (GetChannelMaskByLightFunction(E_HighBeam)&0x02))
    {
        if(lgtctl.st_LgtAct[E_HighBeam]==ACT_ON)
        {
            lgtctl.st_LgtAct[E_LowBeam]=ACT_ON;
        }
    }
}
