...
/* functionsafety mode */
        ...
        {
            LB_E2EHistory=1;
            Interface_SetD900Data(LB_E2E_failure, STATE_ERROR);
        ...
        {
/* normal mode */  
            Interface_SetD900Data(LB_E2E_failure, STATE_OK);          
            if((LB_E2EFlag.bits.ActnOfLedLoBeamCntErr==0)&&(LB_E2EFlag.bits.ActnOfLedLoBeamCrcErr==0)&&(LB_E2EFlag.bits.ActnOfLedLoBeamTimeout==0)&&(LB_E2EHistory==1))
            {
                LB_E2EHistory=0;
                SetLgtStsFb_Status(STS_OFF,E_LowBeam);
            }
...
/* the status of lowbeam */
            if(SwitchOn==ACT_ON)
            {
                ...
                if(Interface_GetChannelState_Light(ChannelID1)==0)  //channel err
                {      
                    if(LB_ErrStatus==0)
                    {
