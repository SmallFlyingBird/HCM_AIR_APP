 ...
 *                                                              *
 ****************************************************************/
E_LgtAct_t Channel_Surfaces_Apparent=ACT_ON;/* ACT_ON CH2????ACT_OFF?CH2????*/
    ...
    for(id=ChannelID1;id<CHANNEL_NUM;id++)
    {
        if(((lgmask>>id)&0x01)!=0) 
        {
            ...
            {/* Right side*/
                ...
                if(TIsts!=0)
                {
                    Channel_Surfaces_Apparent=ACT_OFF;
                }
                else
                {
                    Channel_Surfaces_Apparent=ACT_ON;
                }
            }
            ...
            {
                Interface_SetD900Data(TI_E2E_failure, STATE_ERROR);
                TI_Off(id);
                TI_E2EHistory = 1;
                if(TIsts==ACT_ON)
                {/* when indicator on*/
...
#endif
            {
                Interface_SetD900Data(TI_E2E_failure, STATE_OK);
...
/* ??CH2'????????? */
E_LgtAct_t Interface_Get_Channel_Surfaces_Apparent_Switch(void)
{
    return Channel_Surfaces_Apparent;
}