...
void Interface_SetDtcNtcError(E_NtcSignalNo ntcno, E_NtcErrorType ntcerror, uint8_t val)
{
    ...
    case E_NtcErrorType_OpenOrShort2Vcc:
    {
        ...
        if(val==1)
        {
            Interface_SetD900Data( NTC1_OL+ntcno*3 , STATE_ERROR);
            Interface_SetD900Data( NTC1_SCB+ntcno*3, STATE_ERROR);
        }
        else
        {
            Interface_SetD900Data( NTC1_OL+ntcno*3, STATE_OK);
            Interface_SetD900Data( NTC1_SCB+ntcno*3, STATE_OK);
        }
    }
    break;
    case E_NtcErrorType_Short2Gnd:
    {
        ...
        if(val==1)
        {
            Interface_SetD900Data( NTC1_SCG+ntcno*3, STATE_ERROR);
        }
        else
        {
            Interface_SetD900Data( NTC1_SCG+ntcno*3, STATE_OK);
        }
    }
    break;
    }
...
void Interface_SetDtcBinError(E_BinType BinType, uint8_t val)
{
    if (BinType <= E_BinType_Bin3)
    {
        if (val)
        {
            ...
            Interface_SetD900Data( Rcod1_Fail+BinType, STATE_ERROR);
        }     
        else
        {
            ClearErrorMapValRealTime(gMap_BinError[BinType]);
            Interface_SetD900Data( Rcod1_Fail+BinType, STATE_OK);
        }
    }
}
